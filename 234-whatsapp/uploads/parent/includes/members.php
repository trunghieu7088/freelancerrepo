...
class ET_User extends AE_Base{
	...
	public function _insert($data){
		$data = array_merge($data,array('role'=>'author'));
		$args = $this->_filter_meta($data);
		$role = isset($args['data']['role']) ? $args['data']['role']: 'author';

		if ( strtolower($role) == 'administrator' || strtolower($role) =='editor' ) { //// 2.0.13
			throw new Exception(__('An error occur when created sign up', ET_DOMAIN), 404);
			wp_die('exit');
		}
	...
	public function _update($data){
		try {
			...
			$role = isset( $data['role'] ) ? $data['role']: 'author'; // 2.0.13

			if ( strtolower($role) == 'administrator' || strtolower($role) =='editor' ){
				throw new Exception(__('Unauthenticated error.', ET_DOMAIN), 404);
	...
	protected function _convert($data){
		...
		return apply_filters( 'et_convert_user', $result);
	}
...
class QA_Member extends ET_User{
	...
	static public function convert($user){
		...
		*/
		...
		//custom code here
		$result->university=get_user_meta($result->ID,'university',true);
		$result->level_education=get_user_meta($result->ID,'level_education',true);
		$result->job_role=get_user_meta($result->ID,'job_role',true);
		//end
	...
	static public function get_avatar($id, $size = 64 ,$params = array('class'=> 'avatar' , 'title' => '', 'alt' => '')){
		...
		if ( !empty($avatar) && !empty($avatar['thumbnail']) && isset($avatar['thumbnail'][0])){
			$avatar = '<img itemprop="image" src="'.$avatar['thumbnail'][0].'" class="'.$class.'" alt="'.$alt.'" />';
		} else {
			$link 	= get_avatar( $id, $size );
			preg_match( '/src=(\'|")(.+?)(\'|")/i', $link, $array );
			$sizes = get_intermediate_image_sizes();
			...
			$avatar = '<img itemprop="image" src="'.$avatar['thumbnail'].'" class="'.$class.'" alt="'.$alt.'" />';
		}
		return $avatar;
	}
	...
	public function user_sync(){
		...
		switch ($_POST['method']){
			...
			case 'resend_confirm_email':
				$resp = $this->resend_confirm_email();
				break;
			default:
				break;
		}
		...
		try{
			...
			if( isset($_POST['do_action'])){
				...
				if( $_POST['do_action'] == "saveProfile" ){
					$check_mail = get_user_by('email',$args['user_email'] );
					//check email, if email is already used throw error
					if($user_email == $args['user_email']){
						if($check_mail || $user_email == $args['user_email']){
							QA_Member::update(array(
								...
								//custom code here
								'university' => sanitize_text_field($args['university']),
								'level_education' => sanitize_text_field($args['level_education']),
								'job_role' => sanitize_text_field($args['job_role']),
								//end
							...
							$resp = array(
								'success' 	=> false,
								'msg' 		=> __('This email is already used. Please enter another email!',ET_DOMAIN ),
								'email'		=> $user_email
							);
						}
					}else{
...
//				'success' => false,
//				'code'    => 401,
//				'msg'     => $static['texts']['confirm_account'],
//			);
//		}
		...
		if ( !is_wp_error($user) ){
			$resp = array(
				...
				'redirect' => apply_filters( 'redirect_link_after_logged', get_post_type_archive_link( 'question' ) ),
				'data'     => array(
					'user' 		=> $userdata,
					'nonce' 	=> $nonce
				)
				...
				elseif(isset($_FILES[$fileID])){
					...
					if ( !is_wp_error($attach_id) ){
						...
						try {
							...
							$avatar_url = isset($user_avatar['thumbnail'][0]) ? $user_avatar['thumbnail'][0] : '';
	...
	public function register(){
		...
		try {
			...
			if ( wp_create_nonce('signup_form_' . $param[ 'signup_form_id' ]) != $param['signup_form_nonce'] || ! wp_doing_ajax() ){

				throw new Exception( __("Unauthenticated issue.", ET_DOMAIN) );
				wp_die('unauthenticated');
			}
			...
			//custom code here
			update_user_meta($user_id,'university',$param['university']);
			update_user_meta($user_id,'level_education',$param['level_education']);
			update_user_meta($user_id,'job_role',$param['job_role']);
			//end
	...
	public function resend_confirm_email(){
		...
		if(!empty($register_status) && $register_status == "unconfirm") {
			...
			$resp = array(
				"success" => true,
				"msg" => __("Successfully resend the confirmation email.", ET_DOMAIN)
			);
		} else {
			$resp = array(
				"success" => false,
				"msg" => __("Your account has been activated.", ET_DOMAIN)
			);
		}
	...
	public function inbox(){
		...
		try {
			...
			$subject     = apply_filters('qa_inbox_subject',__('New Private Message From ', ET_DOMAIN).html_entity_decode(get_bloginfo('blogname' )));
			$message     = ae_get_option('inbox_mail_template');
			$new_message = stripslashes(str_replace("\n", "<br>", $args['message'])) ;
			$blogname    = html_entity_decode(get_bloginfo('blogname'));
 ...
 *
 */
function et_register( $userdata, $role = 'author', $auto_login = false ){
	extract($userdata);
