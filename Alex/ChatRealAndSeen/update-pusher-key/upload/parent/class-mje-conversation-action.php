...
class MJE_Conversation_Action extends MJE_Post_Action
{
    ...
    public function after_send_message($message, $request) {
        ...
        if(isset($message['data']) && !empty($message['data'])) {
            ...
            // custom code for convert order message to the notification
                $check_mjob_order=get_post($message_data->post_parent);                
            if($message_data->type == 'message' && $check_mjob_order->post_type=='mjob_order')            
            {
                    $contentmsg=$message_data->post_content;
                    if(strlen($contentmsg) > 35 && str_word_count($contentmsg) <= 4)
                    {
                        $contentmsg=substr($contentmsg,0,30).'...[View more]';
                    }    
                    else
                    {
                        $contentmsg=wp_trim_words(strip_tags($contentmsg),12,'...[View more]');    
                    }
                    
                    $code = 'type=new_order_message';                                        
                    $code .= '&post_id=' . $message_data->ID;
                    $code .= '&mjob_id=' . $check_mjob_order->ID;
                    $code .= '&sender=' . $message_data->from_user;
                    $code .= '&contentmsg=' . $contentmsg;
                    MJE_Notification_Action::get_instance()->create( $code, $message_data->to_user );                    
            }
...
//                Send email when buyer send custom order
            ...
            if($message_data->type == 'offer') 
            {
                ...
                // my custom code 
                if(isset($request['custom_order_id']) && $request['custom_order_id']=='none')
                {
                    $auto_custom_order=array('post_title'=>'auto custom order',
                        'post_content'=>'Offre créée par le vendeur',
                        'post_author'=>$request['to_user'],
                        'post_status'=>'publish',
                        'comment_status'=>'open',
                        'post_name'=>'auto custom order name',                        
                        'post_type'=>'ae_message'
                    );
                   $auto_custom_order_id=wp_insert_post($auto_custom_order);
                   update_post_meta($auto_custom_order_id,'from_user',$request['to_user']);
                   update_post_meta($auto_custom_order_id,'to_user',$request['from_user']);
                   update_post_meta($auto_custom_order_id,'type','custom_order');
                   update_post_meta($auto_custom_order_id,'custom_offer_id',$message_data->ID);
                   update_post_meta($auto_custom_order_id,'custom_order_status','offer_sent');
                

                  // update_post_meta($auto_custom_order_id,'auto_custom_order',1);


                    // can nhac 2 dong nay ( them budget va deadline cho auto custom order)

                   update_post_meta($auto_custom_order_id,'custom_order_deadline',absint($request['etd']));
                   update_post_meta($auto_custom_order_id,'custom_order_budget',absint($request['budget']));

                   //update_post_meta($auto_custom_order_id,'parent_conversation_id',$request['post_parent']);
                   update_post_meta($auto_custom_order_id,'custom_order_mjob',$request['custom_mjob_id_offer']);                
                }
                ...
                // my custom code
                 if($request['custom_order_id']!='none')
                {
