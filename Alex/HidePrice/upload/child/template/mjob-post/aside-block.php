...
<div class="box-shadow box-aside-stat">
    ...
    <div class="mjob-single-stat">
        <div class="stat-block clearfix">
           ...
           <span class="price pull-right">Discuter</span>
    ...
    <div class="action">
        <?php if( $user_ID != $mjob_post->post_author ) {
          //  mje_render_order_button( $mjob_post );
            mje_show_contact_link_mjob($mjob_post->post_author);
              if( $user_ID != $mjob_post->post_author ) {
                ...
                ?>
                <div>
                    <a id="<?php echo $send_custom_order_id; ?>" class="btn-submit <?php echo $class_link_order;?>" data-mjob-name="<?php echo $mjob_post->post_title; ?>" data-mjob="<?php echo $mjob_post->ID ?>" data-conversation-guid="<?php echo $conversation_guid; ?>" data-conversation-parent="<?php echo $conversation_parent; ?>" data-to-user="<?php echo $mjob_post->post_author; ?>" data-from-user="<?php echo $user_ID ?>" style="background-color:#FF5733 !important;border-color:#FF5733 !important;display:block;cursor: pointer"><?php _e('Offre personnalisée','enginethemes'); ?></a>
                </div>
                <?php
            }
			mje_render_buy_fee();
...
<?php
$audio_meta = wpautop($mjob_post->audio_meta);
$file_descriptions = wpautop($mjob_post->file_descriptions);
if($mjob_post->opening_message && $mjob_post->opening_message != '' && ($user_ID == $mjob_post->post_author || is_super_admin())) : ?>
