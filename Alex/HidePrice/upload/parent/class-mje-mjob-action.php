...
class MJE_MJob_Action extends MJE_Post_Action
{
    ...
    public function sync(){
        $_POST['file_descriptions']=@implode('|*|',$_POST['file_descriptions']);
        $request = $_POST;
...
/*print_r($_FILES);die;*/
        ...
        if (isset($response['data']) && !empty($response['data'])) {
            ...
            if($result->audio_meta) {
                $response['data']->audio_meta = $result->audio_meta;
            }
            if($result->file_descriptions) {
                $response['data']->file_descriptions = $result->file_descriptions;
            }
    ...
    public function convert($result){
       ...
       // $result->et_budget_text = mje_shorten_price($result->et_budget);
         $result->et_budget_text = 'Discuter';
        //end
        ...
        //custom code for online offline list
        $profile_id_custom=get_user_meta($result->post_author,'user_profile_id',true);
        $custom_online_status=get_post_meta($profile_id_custom,'custom_online_status',true) ? get_post_meta($profile_id_custom,'custom_online_status',true) : 'offline';  
        ...
        $result->online_status_text=$online_status_text;
        $result->style_status=$style_status;
        $result->style_div= $style_div;
        //end
