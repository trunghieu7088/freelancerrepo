...
//custom code role feature
$custom_current_user=wp_get_current_user();
if ( in_array( 'expert', $custom_current_user->roles, true ) ) {
    $is_expert=true;
}
else
{
...
?>
                        ...
                        <!-- custom code send offer here + custom role -->
                        <?php if($is_expert): ?>
                            <?php if(!$custom_order_query->have_posts()) : ?>
                            <button style="margin-left:20px;margin-bottom:10px;background-color:#10a2ef !important;border-radius: 4px !important;" class="btn-send-offer" data-custom-order="none"><?php _e('Send offer', 'enginethemes') ?></button>
                            <!-- <a style="background-color:#27ae60 !important;border-color: #ff5733 !important;border:none;outline:none;padding:5px 20px;border-radius: 4px !important;font-size:12px;color:#fff;cursor: pointer;display:inline-block;margin-left:20px;margin-bottom:10px;" id="bt-send-custom" class="bt-send-custom choose-mjob-conversation"  data-mjob-name="" data-mjob="na" data-conversation-guid="<?php echo $custom_guid; ?>" data-conversation-parent="<?php echo $current->id;?>" data-active-conversation="active" data-to-user="<?php echo $receiver_id; ?>" data-from-user="<?php echo $user_ID; ?>">Send custom order</a> -->
