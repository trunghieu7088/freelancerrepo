...
if (!function_exists('ae_pagination')) :
    ...
    function ae_pagination($query, $current = '', $type = 'page', $text = '')
    {
        ...
        $query_var['order'] = isset($query->query_vars['order']) ? $query->query_vars['order'] : NULL;
        ...
        if (($max_num_pages <= 1 && !et_load_mobile()) || !$type) return;
        $style = '';
        if (et_load_mobile() && $max_num_pages <= 1) {
...
if (!function_exists('ae_user_pagination')) :
     ...
     * @author Dakachi
     */
    function ae_user_pagination($query_args = array(), $total = 0, $current = '', $echo = true)
    {
        $str_paginate =  '';
...
if (!function_exists('ae_favicon')) {
     ...
     * @return void
     */
    function ae_favicon($favicon_path = "/img/favicon.png")
    {
        $img = get_template_directory_uri() . $favicon_path;
        $options = AE_Options::get_instance();
        // save this setting to theme options
...
if (!function_exists('ae_tax_dropdown')) {
     ...
     * @author Dakachi
     */
    function ae_tax_dropdown($tax, $args = array(),$custom_parent = false)
    {
        //custom code here
        if($custom_parent==true)
        {
            $defaults = array(
                ...
                'hide_if_empty' => false, 'attr' => '','parent'=>0
            );
