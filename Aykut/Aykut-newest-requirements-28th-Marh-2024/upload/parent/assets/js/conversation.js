        ...
        Views.SingleConversation = Backbone.View.extend({
			...
			sendMessageClick: function(e){
                ...
                //if( e.keyCode == 13 && !e.shiftKey) {
                    ...
                    //custom code filter bad words frontend 28th mar 2024
            let forbiddenWords = custom_bad_words.split(',').map(function(word) {
              return word.trim(); // Remove any leading/trailing whitespaces
             });
    
             let check_message = message.toLowerCase();
             let check_pattern = new RegExp(forbiddenWords.join('|'), 'gi');
             if (check_pattern.test(check_message)) {
              // If a forbidden word is found, alert the user
              toastr.error('Your message includes forbidden content !!!');
                return;
              } 

              let phoneNumberPattern = /\b\d{4,}\b/;

              if (phoneNumberPattern.test(check_message)) {
                // If a forbidden word is found, alert the user
                toastr.error('Your message includes forbidden content !!!');
                  return;
                } 
