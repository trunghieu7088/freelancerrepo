...
if ( $registered_seller ) {
	...
	if ( 'yes' !== get_user_meta( $user_data->ID, 'mje_opp_account_created', true ) ) {
		...
		$args = array(
			...
			'type'         => in_array( $seller_type, array( 'enterprise', 'soletrader', 'freelancer' ) ) ? 'business' : 'consumer',
		...
		if ( in_array( $seller_type, array( 'enterprise', 'soletrader', 'freelancer' ) ) ) {

			$args['coc_nr'] = $profile->billing_vat;
					...
					<div class="block-payment">
					   ...
					   <!-- <p class="title"><?php // _e('Verkäuferstatus', 'enginethemes'); ?></p> -->
						...
						if ( 'yes' !== get_user_meta( $user_data->ID, 'mje_opp_account_created', true ) ) {
							...
							$account_creation_error_response = get_user_meta( $user_data->ID, 'mje_opp_account_creation_error_response', true );

							$account_creation_error_response = json_decode( $account_creation_error_response, true );



							if ( siar( $account_creation_error_response, 'error' ) ) {

								echo '<ul class="error">';

								echo siars( $account_creation_error_response, 'error/message' ) ? '<li>' . siars( $account_creation_error_response, 'error/message' ) . '</li>' : '';

								if ( siars( $account_creation_error_response, 'error/parameters' ) ) {

									foreach ( siars( $account_creation_error_response, 'error/parameters' ) as $error_key => $error ) {

										echo '<li>' . siar( $error, 0 ) . '</li>';

									}

								}

								echo '<li>Please check details and you can retry from &nbsp; <a href="' . add_query_arg( 'opp_profile', 'create', et_get_page_link( 'profile' ) ) . '"> here</a></li>';
						...
						<ul>
							...
							<li style="padding-left:30px;margin-bottom:25px;">								
								...
								<?php if ( $seller_type == 'enterprise' || $seller_type == 'soletrader' || $seller_type == 'freelancer' ) : ?>
						   ...
						   <div class="block-billing" style="margin-top:20px !important;padding-left:30px;">
							...
							<ul>
								...
								<li>
									...
									<div class="cate-title" style="padding-top:10px;flex-basis: 300px;">
										...
										if ( $seller_type == 'soletrader' || $seller_type == 'enterprise') {

											echo 'Handelsregisternummer';

										} else {

											echo 'Umsatzsteuernummer';
									...
									<div id="billing_vat" class="info-content" style="padding-left:0px;margin-left: 10px;">
