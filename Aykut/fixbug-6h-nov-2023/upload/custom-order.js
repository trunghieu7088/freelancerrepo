    ...
    $(document).ready(function() {
            ...
            initialize: function () {
                ...
                else
                {
                    $('.custom-sendorder-form-modal :input').attr('disabled','disabled');
                     $('.mjob-name').html('Anfrage stellen');	
        ...
        if($('.box-content-custom-order').length > 0) {
            ...
            Views.CustomOrderDetail = Backbone.View.extend({
                ...
                showOuterDetail: function(openOfferForm=false) {
                    ...
                    //30th oct 2023 phai mo lai code vi co description dai tren desktop se co bug
                    /* $(".outer-detail-custom").mCustomScrollbar({
                                theme:"dark",
                    ...
                    //6th nov 2023 fix bug tiep
                       if(openOfferForm==true)
                       {
                            //only activate this for mobile
                           
                             if(window.innerWidth < 768) {
                                  $(".outer-detail-custom").css('height','600px');
                                 $(".outer-detail-custom").mCustomScrollbar({
                                         theme:"dark",
                                         contentTouchScroll: true,
                                         scrollInertia:600,                               
                       ...
                       {
                            $(".outer-detail-custom").css('height','750px');
                            $(".outer-detail-custom").mCustomScrollbar({
                                theme:"dark",
                                contentTouchScroll: true,
                                scrollInertia:600,                               
                ...
                showSendOfferForm: function (event) {
                    event.preventDefault();
                     //custom code 17th Oct 2023 auto get information for offer from custom order
                    //let url_ajax=window.location.origin+'/wp-admin/admin-ajax.php';
                    var customOrderID = $(event.currentTarget).attr('data-custom-order');
                    if(customOrderID !== 'null' && customOrderID !=='undefined' && customOrderID !== null)
                    {
                        Backbone.ajax({
                            dataType: "json",
                            type: "post",
                            url: ae_globals.ajaxURL,
                             data : 
                             {
                                action: "get_info_custom_order",           
                                customOrderIDSent : customOrderID,                                         
                            },
                            success: function(response){  
                                                                                                
                                   if(response.success==true || response.success =='true')
                                   {
                                        $("#etd").val(response.etd);
                    ...
                    $('.outer-detail-custom-order .action-form').hide();
                    $('#send-offer').show();
                    //custom code 6th Nov 2023
                    var openOfferForm=true;
                    this.showOuterDetail(openOfferForm);
                    //end
