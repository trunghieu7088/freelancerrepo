<?php
if( ! class_exists('FRE_Credit_History')):
...
class Fre_Credit_HistoryAction extends AE_PostAction
{
    ...
    public function fre_credit_convert_history($result){
        ...
        switch($post_title){
           //custom code here
            case 'Project':
           // $extra_option=get_post_meta($result->ID,'history_type',true);
             $custom_info_changelog=get_post_meta($result->ID,'info_changelog',true);
          //  $result->info_changelog='Pay extra option: '.$extra_option ;
              $result->info_changelog=$custom_info_changelog;
            //end
            ...
            case 'Received':
                ...
                break;
            case 'Add': // fre_credit_plus
                $result->post_title = __('Receive');
                $result->info_changelog = 'Added'; // action
                $payment_gateway = $payment;
                $result->amount_text = fre_price_format($result->amount);
                break;
            case 'Deducted': // fre_credit_plus
                $result->post_title = __('Pay');
                $result->info_changelog = 'Deducted';
                $payment_gateway = $payment;
                $result->amount_text = fre_price_format($result->amount);
                break;
            case 'Subscribe': // fre_credit_plus
                $payment_gateway = __('Credit system',ET_DOMAIN);
                $result->post_title = __('Subscribe ');
                $result->info_changelog = 'subscribe';
                $sku = get_post_meta( $result->ID, 'subscribe_pack_sku', true);
                $pack_type = "bid_plan";
                if( ae_user_role($result->post_author) !== 'freelancer'){
                    $pack_type = 'pack';
                }
                ...
                $pack = current($pack);
                if(!empty($pack))
                    $result->info_changelog = sprintf( __('Subscribe plan %s ', ET_DOMAIN), $pack->post_title);

                $result->amount_text = fre_price_format($result->amount);
                break;
        }
        $result = apply_filters('render_credit_history_item', $result, $post_title);
        ...
        $result->history_status_text = 'NA';
        if( isset($arr_status[$result->history_status]) )
