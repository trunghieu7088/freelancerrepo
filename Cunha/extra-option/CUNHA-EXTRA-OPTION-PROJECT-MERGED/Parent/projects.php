...
class Fre_ProjectAction extends AE_PostAction {
	function __construct( $post_type = 'project' ) {
		...
		$this->disable_plan = is_post_project_free();
		...
		if ( isset( $query['orderby'] ) ) {
			...
			switch ( $orderby ) {
				...
				case 'urgent':
					$query_args['orderby'] = 'date';
					$query_args['order']   = 'DESC';
					$query_args['meta_key'] = 'custom_extra_option';
					break;	
					//end custom code extra option here
		...
		if ( $method == 'archive' ) {
			...
			if ( isset( $project_id ) && $project_id != '' ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					$message   = __( "Your project has been approved!", ET_DOMAIN );
			...
			if ( ( isset( $project_id ) && $project_id != '' ) && ( isset( $request['reject_message'] ) && $request['reject_message'] != '' ) ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					$permalink = get_permalink( $result );
					$message   = __( "Your project has been rejected!", ET_DOMAIN );
		...
		if ( ! is_wp_error( $result ) ) {
			...
			if ( isset( $request['et_payment_package'] ) ) {
				...
				if ( $check['success'] ) {
					...
					// next step to checkout or redirect to detail of project has been posted.
				}
			...
			if ( $request['method'] == 'create' ) {
				...
				//custom code here
			//	update_post_meta($result->ID,'test_field',$request['urgent_extra_option']);
				$user_wallet 	= FRE_Credit_Users()->getUserWallet(get_current_user_id());
				if(isset($request['realcost']) && $request['realcost'] != '')
				{
					if($request['realcost']=='private')
					{
						$price_extra=get_option('privateprice');
					}
					if($request['realcost']=='urgent')
					{
						$price_extra=get_option('urgentprice');
					}

					if($user_wallet->balance >= $price_extra)
					{
						//xu ly khi la urgent
						if($request['realcost'] =='urgent')
						{
							update_post_meta($result->ID,'custom_extra_option',$request['realcost']);
						}
						//xu ly khi la private
						if($request['realcost'] =='private')
						{
							update_post_meta($result->ID,'private_extra_option',$request['realcost']);
						
						}


						$user_wallet->balance = $user_wallet->balance - $price_extra;
						FRE_Credit_Users()->setUserWallet(get_current_user_id(), $user_wallet);
						...
						$history_post = array(
					        ...
					        'post_title' => 'Project',
					        'post_content' => 'charge ' . $price_extra
					    );
					    ...
					    {
							update_post_meta( $history_id, 'history_type', $request['realcost']);
					    	update_post_meta( $history_id, 'amount', $price_extra );
					    	...
					    	update_post_meta( $history_id, 'info_changelog', 'Pay extra option: '.$request['realcost']);
					    }
					    //end save
					    ...
					    $profile_name=get_userdata(get_current_user_id());
					     $content='<h3>'.$emailcontent.' </h3>';  
					     $content.='<p><a href="'.site_url('/my-credit/').'">'.$credithistory.'</a></p>';
					     $mailer_custom->wp_mail( $profile_name->user_email,
                  $emailtitle,$content);
					     update_option('newlog',$content);
					}
	...
	function ae_convert_project( $result ) {
		...
		//custom code for extra option here
		if(get_post_meta($result->ID,'custom_extra_option',true)=='urgent')
		{
			$result->custom_urgent=true;
			$result->custom_urgent_text=get_option('urgentlabel');
		}
		else
		{
			$result->custom_urgent=false;
			$result->custom_urgent_text=get_option('urgentlabel');
		}
	...
	public function validate_data( $data ) {
		...
		if ( isset( $data['et_payment_package'] ) ) {
			...
			$sku = $data['et_payment_package'];
			// $plan = AE_Package::get_instance()->get_pack($sku, 'pack');
			$plan = fre_get_pack($sku);  // v 1.8.19

			if ( !$is_post_free &&  ! $plan ) {
				if(!$plan)
