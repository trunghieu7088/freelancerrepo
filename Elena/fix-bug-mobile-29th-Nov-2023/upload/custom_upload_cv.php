<?php
    ...
    add_action('et_mobile_head','add_upload_custom_cv_script_mobile',999,0);

    function add_upload_custom_cv_script_mobile()
    {       
        global $isMobile;
        if($isMobile)
        {
        ?>        
        <link rel='stylesheet' id='custom-upload-cv-style-mobile-css' href="<?php echo get_stylesheet_directory_uri().'/assets/css/custom-upload-cv.css?ver=1.0' ?>" type='text/css' media='all' />       
        <link rel='stylesheet' id='custom-fontawesome-mobile' href="<?php echo get_stylesheet_directory_uri().'/assets/fontawesome/css/font-awesome.min.css?ver=1.0' ?>" type='text/css' media='all' />       
        <?php
        }
    }

    add_action('wp_head','add_custom_upload_js_mobile',99,0);

    function add_custom_upload_js_mobile()
    {   
        global $isMobile;
        if($isMobile)
        {     
        ?>
        <script type="text/javascript" src="<?php echo get_stylesheet_directory_uri() . '/assets/js/custom_upload_cv.js'; ?>"></script>
        <?php
        }
    }
