...
if( !je_allow_view_resume() || (!$user_ID && $resume_options->et_resumes_priavcy) ) {
	...
	<div data-role="content" class="resume-contentpage">
		<h1 class="title-resume">
			...
			<?php if(current_user_can( 'manage_options' ) ||  $resume->post_author == $user_ID) { ?>
				<span class="step-number"><a href="<?php echo esc_url(add_query_arg(array ('edit' => $post->ID) )); ?>" ><?php _e("Edit", ET_DOMAIN); ?> </a></span>
			<?php } ?>
		...
		<div class="content-info content-text">
			<a data-ajax="false" href="<?php echo esc_url(add_query_arg('action', 'send_message')) ?>" data-role="button" class="btn_contact"><?php _e("CONTACT", ET_DOMAIN); ?></a>
		</div>
		<?php } ?>
	...
	// sending message
		...
		<div data-role="content" class="post-content">
			...
			<form action="" id="jobseeker_message" data-ajax="false" method="post">
				...
				<input type="hidden" id="cancel_url" value="<?php echo esc_url(remove_query_arg( 'action' )) ?>">
