...
<div class="wrapper jobseeker">
	<?php if( je_allow_view_resume() && ( $user_ID || !$resume_options->et_resumes_priavcy) ) { //check privacy hide resume detail  ?>
		...
		<div id="resume_profile" class="jse-account main-column jse-profile-main jse-signup-profile">
			...
			*/
            ...
            <div class="module module-edit education">
				...
				<?php 
					$cv_id=get_post_meta($resume->ID,'custom_cv_attach_id',true);
					$cv_link= wp_get_attachment_url($cv_id);
					$cv_info=get_post($cv_id);
					//var_dump($cv_info);
				?>				

				<?php if($cv_link  && !$authorise) : ?>
				<div class="cv-show-area">					
					<a href="<?php echo $cv_link; ?>" download><i class="fa fa-download" aria-hidden="true"></i> Scarica il Curriculum</a>													
				</div>
				<?php endif; ?>

				<?php if ( $authorise  && $cv_link): ?>
					<div class="cv-show-area">	
						<a href="<?php echo $cv_link; ?>" download><i class="fa fa-download" aria-hidden="true"></i> <?php echo $cv_info->post_name; ?></a>																			
					</div>
				<?php endif; ?>

				<?php if ( $authorise): ?>
                <div class="edu-skill cnt" id="cv-upload-container" name="cv-upload-container">
				   	<?php if(!$cv_link) : ?>
						<p>Puoi caricare il Curriculum in .pdf qui</p>
					<?php else: ?>
						<p>Caricando un nuovo .pdf sostituirai il .pdf esistente</p>
					<?php endif; ?>
                    <input type="hidden" id="custom-upload-cv-nonce" name="custom-upload-cv-nonce" value="<?php echo wp_create_nonce('custom-upload-cv-nonce'); ?>">
					<input type="hidden" id="resumeID" name="resumeID" value="<?php echo $resume->ID; ?>">
					<a href="javascript:void(0)" id="custom-upload-pdf-btn" name="custom-upload-pdf-btn" class="btn-custom-upload-pdf"><i class="fa fa-upload" aria-hidden="true"></i> Carica Curriculum</a> 
                </div>
				<?php endif; ?>
            </div>
            <!-- end custom code -->
			...
			<div class="widget sortable ui-sortable">
				<aside class="widget bg-grey-widget jobseeker-profile-widget">
						...
						<div class="info">
							<div class="display">
								<div id="profile_urls">
									...
									$urls = apply_filters( 'je_jobseeker_info_url', $urls );
									do_action( 'je_before_info_url', $urls);
									...
									do_action( 'je_after_info_url', $urls);
									?>
								</div>
								<?php if ($authorise){ ?>
									<div class="btn-save">
					...
					<div class="btn-jse-contact">
						<a href="<?php echo esc_url(add_query_arg('action', 'send_message')) ?>" class="bg-btn-action border-radius btn">
				...
				<form action="" id="jobseeker_send_message" method="post">
					<div class="jse-message">
						...
						<div class="jse-submit">
							...
							<a id="cancel_form" class="button-cancel" href="<?php echo esc_url(remove_query_arg( 'action' )) ?>"><?php _e('Cancel', ET_DOMAIN) ?> <span data-icon="D" class="icon"></span><span class="line-bottom"></span></a>
						</div>
	...
	<?php } else  if ( !je_allow_view_resume() ){
		?>
			<div  class="main-center clearfix padding-top30 jse-profile">
				<?php _e('You don\'t have permission to view this resume',ET_DOMAIN); ?>
			</div>
		<?php
