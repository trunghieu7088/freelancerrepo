...
function disable_button_add_new() {
	...
	if ( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'fre_profile' ) {
		echo '<style type="text/css">#favorite-actions1, .add-new-h2, .page-title-action { display:none; } .admin-bar post-type-fre_profile .tablenav{display:inherit;} </style>';
	}
}
...
class Fre_ProfileAction extends AE_PostAction {
	...
	function ae_convert_profile( $result ) {
		...
		$result->earned =  sprintf( __(' %s earned', ET_DOMAIN ), price_about_format( $earned ) );
		$result->excerpt = fre_trim_words( $result->post_content, 80 ); // 1.8.3.1
		//custom code here
		$result->custom_rank_order=get_post_meta($result->ID,'custom_rank_order',true);
		//$result->total_freelancer_has_rank=get_option('total_freelancer_has_rank');

		//custom code new profile
		$result->rawearnd=$earned;
		$result->total_projects_worked_count=$project_worked;
			...
			if ( isset( $query['orderby'] ) ) {
				...
				switch ( $orderby ) {
					...
					case 'rating':
						...
						//custom code here
					case 'custom_rank_order':
						$query_args['meta_key']     = 'custom_rank_order';
		...
		if ( ! is_wp_error( $result ) ) {
			...
			if ( $request['method'] == 'create' ) {
				...
				$profile_id = get_user_meta( $user_ID, 'user_profile_id', true ); // 1.8.6.1
					...
					//custom code rank here
				$user_role_get=get_user_by('id',$user_ID);
				$user_role_array=$user_role_get->roles;
				if(in_array('freelancer',$user_role_array))
				{
					$custom_rank_order=get_post_meta($profile_id,'custom_rank_order',true);
					if(empty($custom_rank_order))
					{
						$lowest_rank = apply_filters( 'get_the_lowest_rank_filter', 'lowest_rank' );
						$lowest_rank+=1;
						$datetime_rank_update = date('Y-m-d H:i:s'); 
						update_post_meta($profile_id,'custom_rank_order',$lowest_rank,true);
						update_post_meta($profile_id,'custom_rank_point',0,true);
						update_post_meta($profile_id,'custom_rank_update_time',$datetime_rank_update,true);

						update_user_meta($user_ID,'custom_rank_order',$lowest_rank,true);
						update_user_meta($user_ID,'custom_rank_point',0,true);
						update_user_meta($user_ID,'custom_rank_update_time',$datetime_rank_update,true);
					}
				}
...
class Fre_PortfolioAction extends AE_PostAction {
	...
	function ae_convert_portfolio( $result ) {
		...
		$result->the_post_thumbnail = $result->the_post_thumbnail_full = '';

		if($thumbnail_full_src && is_array($thumbnail_full_src)){
		...
		if ( ! empty( $list_image_portfolio ) ) {
			...
			$result->list_image_portfolio[] = array( 'id'    => get_post_thumbnail_id( $result->ID ),
			                                         'image' => $result->the_post_thumbnail_full,
			);
		}
