...
class Fre_BidAction extends AE_PostAction {
	...
	public function fre_notify_admin_bid( $data ) {
		...
		et_track_payment('START_SEND_MAIL ==> new payment email & Add new payment notification').
		$this->mail->new_payment_notification_mail( $data['order_id'] );
		Fre_Notification::getInstance()->new_payment_notification_onsite($data);
		et_track_payment('SEND_MAIL_NOTIFICATON_COMPLETED');
	}
	...
	*/
	function fre_check_before_insert_bid( $args ) {
		...
		*/
		...
		// v1.8.16
		if( ! can_user_bid() ){
			return new WP_Error( 'zero_bid', __( 'Please purchase bids to bid on this project.', ET_DOMAIN ) );
		}
	...
	*/
	function fre_update_after_bidding( $bid_id ) {
		...
		//custom code 2
		$bid_author=get_post_field('post_author',$bid_id,'db');
		$bid_author_profile_id=get_user_meta($bid_author,'user_profile_id',true);
		//$user_role_get=get_user_by('id', $bid_author);
		$custom_options=get_option('et_options');
		
		$rank_bid_condition=$custom_options['make_bid_rank_condition'];
		
		$rank_bid_point=$custom_options['make_bid_rank_points'];
		
		$current_rank_point=get_user_meta($bid_author,'custom_rank_point',true);

		$new_rank_point=$current_rank_point+$rank_bid_point;
		$datetime_rank_update = date('Y-m-d H:i:s'); 
			if($rank_bid_condition ==1)
			{
				update_user_meta($bid_author,'custom_rank_point',$new_rank_point);
				update_user_meta($bid_author,'custom_rank_update_time',$datetime_rank_update);
				update_post_meta($bid_author_profile_id,'custom_rank_point',$new_rank_point);
				update_post_meta($bid_author_profile_id,'custom_rank_update_time',$datetime_rank_update);
			}
			else
			{
				$is_process_rank_bid=get_post_meta($bid_author_profile_id,'is_process_rank_bid',true) ? get_post_meta($bid_author_profile_id,'is_process_rank_bid',true) : 0 ;
					$is_process_rank_bid+=1;

					if($is_process_rank_bid >= $rank_bid_condition)
					{
						...
						//reset lai process
						$is_process_rank_bid_update=$rank_bid_condition - $is_process_rank_bid;
						update_post_meta($bid_author_profile_id,'is_process_rank_bid',$is_process_rank_bid_update);
					}
					else
					{
						// xu ly truong hop so diem process chua dat yeu cau
						update_post_meta($bid_author_profile_id,'is_process_rank_bid',$is_process_rank_bid);
					}
			}
		...
		if ( ae_get_option( 'pay_to_bid', false ) ) {
			/*
			 * kiểm tra  bid free trước, sau tới credit_number(pay_to_bid) và cuối cùng là subscription của membership
			*/
			...
			if( $number_free_bid > 0 ){ // add from 1.8.9
				...
				$number_credit =  get_user_credit_number( $user_ID );
				$used_credit_to_bid = 0;
				if($number_credit){
					$pay_credit = - 1; // -1 time bidding.
					update_credit_number( $user_ID, $pay_credit );
					$used_credit_to_bid = 1;
				}
				do_action('fre_update_bid_left', $used_credit_to_bid);// check and only  update  bid left if $number_credit < 0;
			}
		}
		wp_send_json( array(
			'success' => true,
			'msg'     => __( 'You have successfully biddennnnn.', ET_DOMAIN )
		) );
	}
...
function use_paypal_to_escrow() {
	...
	if ( ae_get_option( 'use_escrow' ) ) {
		...
		if ( isset( $stripe_api['use_stripe_escrow'] ) && $stripe_api['use_stripe_escrow'] ) {
			...
			// version 1.8.16 added this check.
			$flag = is_show_pp_adaptive();
