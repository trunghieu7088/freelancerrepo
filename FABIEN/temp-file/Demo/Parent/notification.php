...
class Fre_Notification extends AE_PostAction
{
	...
	function insert($notification)
	{
		...
		if ( $notify ) {
			...
			//custom code here
			$user_role_get=get_user_by('id', $notification['post_author']);
			$user_role_array=$user_role_get->roles;
			if(in_array('freelancer',$user_role_array))
			{
					$custom_options=get_option('et_options');
					$profile_user_id=get_user_meta($notification['post_author'],'user_profile_id',true);
					$rank_notification_condition=$custom_options['number_of_notification_rank_condition'];
					$rank_notification_point=$custom_options['number_of_notification_rank_points'];

					$current_rank_point=get_user_meta($notification['post_author'],'custom_rank_point',true);
					$new_rank_point=$current_rank_point+$rank_notification_point;
			
				$datetime_rank_update = date('Y-m-d H:i:s'); 
				if($rank_notification_condition ==1)
				{								
					update_user_meta($notification['post_author'],'custom_rank_point',$new_rank_point);
					update_user_meta($notification['post_author'],'custom_rank_update_time',$datetime_rank_update);
				...
				{		
					$is_process_rank_notification=get_post_meta($profile_user_id,'is_process_rank_notification',true) ? get_post_meta($profile_user_id,'is_process_rank_notification',true) : 0 ;
					$is_process_rank_notification+=1;

					//diem process la so diem can dat duoc de lay rank point
					//vi du dieu kien la 10 review de lay 1 rank point thi diem process phai = 10 moi dc 1 rank point
					// va khi dat du dieu kien thi diem process se duoc reset ve 0

					if($is_process_rank_notification >= $rank_notification_condition)
					{
						...
						//reset lai process
						$is_process_rank_notification_update=$rank_notification_condition - $is_process_rank_notification;
						update_post_meta($profile_user_id,'is_process_rank_notification',$is_process_rank_notification_update);
					}
					else
					{
						// xu ly truong hop so diem process chua dat yeu cau
						update_post_meta($profile_user_id,'is_process_rank_notification',$is_process_rank_notification);
					}
				}
	...
	function fre_notify_item($notify)
	{
		...
		if (!in_array($type, $no_project_types) && !($type == "new_private_message" && isset($project) && (int)$project == 0)) {
		...
		switch ($type) {
			...
			// custom code for verify validation
					case 'verify_validation':

			$validation_link=site_url('/validation-page/');
				
				$message        = sprintf( __( 'Admin has approved your validation document', ET_DOMAIN ));
				
				
					if ( ! isset( $sender ) ) {
					$sender = 1;
				}
				$content        .= '<a class="fre-notify-wrap" href="' . $validation_link . '">
			...
			//end
				...
				// custom code for reject validation
					case 'reject_validation':

			$validation_link=site_url('/validation-page/');
				
				$message        = sprintf( __( 'Admin has reject your validation document. Reason :', ET_DOMAIN ));
				...
				$content        .= '<a class="fre-notify-wrap" href="' . $validation_link . '">
                                 <span class="notify-avatar">' .'<span class="noti-icon"><i class="fa fa-info-circle"></i></span>'. '</span>
                                <span class="notify-info">' . 	$message.get_post_field('post_title',$notify->ID). '</span>
                                <span class="notify-time">' . sprintf( __( "%s on %s", ET_DOMAIN ), get_the_time( '', $notify->ID ), get_the_date( '', $notify->ID ) ) . '</span>
                            </a>';
				break;	
			...
			case 'resolve_project':
				...
				$message = sprintf(
					__('%s resolved the disputed project %s', ET_DOMAIN),
			...
			case 'report_dispute_project':
				...
				$message = sprintf(
					__('%s sent you a report on %s', ET_DOMAIN),
			...
			case 'approve_order':
				...
				$message = sprintf(
					__('%s approved your payment on %s package', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
			...
			case 'cancel_order':
				...
				$message = sprintf(
					__('%s declined your payment on %s package', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
					'<strong>' . $package['NAME'] . '</strong>'
				);
			...
			case 'reject_project':
				...
				$message = sprintf(
					__('%s rejected your project %s', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
			...
			case 'archive_project':
				...
				$message = sprintf(
					__('%s archived your draft project %s', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '">
			...
			case 'delete_project':
				...
				$message = sprintf(
					__('%s deleted your archived project %s', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '">
			...
			case 'publish_project':
				...
				$message = sprintf(
					__('%s approved your project %s', ET_DOMAIN),
					'<strong class="author-admin">' . get_the_author_meta('display_name', $admin) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '">
			...
			case 'close_project':
				...
				$message       = sprintf(
					__('%s closed your working project %s', ET_DOMAIN),
			...
			case 'quit_project':
				...
				$bid_author    = get_post_field('post_author', $bid_id);
				$message       = sprintf(
					__('%s discontinued on your project %s', ET_DOMAIN),
			...
			case 'delete_bid':
				...
				$message = sprintf(
					__('%s cancelled his bid on your project %s', ET_DOMAIN),
			...
			case 'new_bid':
				...
				$message    = sprintf(
					__('%s bid on your project %s', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $bid_author) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content    .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '">
			...
			case 'bid_accept':
				...
				$project_owner = get_post_field('post_author', $project);
				$message       = sprintf(
					__('%s accepted your bid on the project %s', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $project_owner) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content       .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '">
			...
			case 'complete_project':
				...
				$project_owner = get_post_field('post_author', $project);
				$message       = sprintf(
					__('%s finished your working project %s', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $project_owner) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
			...
			case 'review_project':
				...
				$message      = sprintf(
					__('%s reviewed on your project %s', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $bid_author) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content      .= '<a class="fre-notify-wrap" href="' . get_permalink($project) . '?workspace=1' . '">
			...
			case 'new_message':
				...
				$message        = sprintf(
					__('%s sent you a message on %s workspace ', ET_DOMAIN),
			...
			case 'new_invite':
				...
				$message = sprintf(
					__('%s invited you to join project %s', ET_DOMAIN),
			...
			case 'new_comment':
				...
				$workspace_link = add_query_arg(array('workspace' => 1), get_permalink($project));
				$message        = sprintf(
					__('%s commented on your project %s', ET_DOMAIN),
			...
			case 'lock_file_project':
				...
				$workspace_link = add_query_arg(array('workspace' => 1), get_permalink($project));
				$message        = sprintf(
					__('%s locked the file section in the project %s. Click here for details', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $sender) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content        .= '<a class="fre-notify-wrap" href="' . $workspace_link . '">
			...
			case 'unlock_file_project':
				...
				$workspace_link = add_query_arg(array('workspace' => 1), get_permalink($project));
				$message        = sprintf(
					__('%s unlocked the file section in the project %s. Click here for details', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $sender) . '</strong>',
					'<strong>' . get_the_title($project) . '</strong>'
				);
				$content        .= '<a class="fre-notify-wrap" href="' . $workspace_link . '">
			...
			case 'new_payment': // 1.8.8
				...
				$message        = sprintf(
					__('%s made a payment . Click here for details', ET_DOMAIN),
			...
			case 'project_status': // 1.8.8
				...
				$message        = sprintf(
					__('Your project is publish . Go to project details', ET_DOMAIN),
					'<strong>' . get_the_author_meta('display_name', $owner) . '</strong>'
				);
				if ($status == 'archive') {
					$message        = sprintf(
						__('Your project is archive . Go to project details', ET_DOMAIN),
						'<strong>' . get_the_author_meta('display_name', $owner) . '</strong>'
					);
				}
...
function fre_user_notification($user_id = 0, $page = 1, $showposts = 10, $class = "dropdown-menu dropdown-menu-notifi dropdown-keep-open notification-list")
{
	...
	if ($class != 'fre-notification-list') {
		...
		echo '<a class="view-more-notify" href="' . et_get_page_link('list-notification', false) . '">' . __('See all notifications', ET_DOMAIN) . '</a>';
		echo '</li>';
	}
	echo '</ul>';
