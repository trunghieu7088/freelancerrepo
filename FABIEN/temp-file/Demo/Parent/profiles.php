...
<?php
		...
		//custom code here for validation
		$args_validation=array('post_type'=>'validation',
                       				 'post_status'=>'publish',
                        			'author'=>$result->post_author,
                                        );
                                    $validated_document=get_posts($args_validation);
            if(!empty($validated_document) && get_post_meta($validated_document[0]->ID,'approve_status',true) == 'publish' )
             {
                $result->validation_item='<img style="width:32px;height:32px;" src="'.get_stylesheet_directory_uri().'/assets/img/verified.png'.'"><em style="font-size:12px;color:#008000;"> Verified</em>'; 
              }          
              else
              {
              	 $result->validation_item=false;
              }                   
		//
		...
		if (isset($request['work_experience']) && !empty($request['work_experience']) && is_array($request['work_experience'])) {
			...
			if (!empty($experience['title']) && !empty($experience['subtitle'])) {
				if (!empty($experience['id'])) {
					...
					unset($experience['id']);
					global $wpdb;
					$update = $wpdb->update($wpdb->postmeta, array('meta_value' => addslashes(serialize($experience))), array('meta_id' => $meta_id));
				} else {
					$update = add_post_meta($profile_id, 'work_experience', addslashes(serialize($experience)));
				}
				if ($update === false) {
					wp_send_json(array(
						'success' => false,
		...
		if (isset($request['certification']) && !empty($request['certification']) && is_array($request['certification'])) {
			...
			if (!empty($certification['title']) && !empty($certification['subtitle'])) {
				if (!empty($certification['id'])) {
					...
					global $wpdb;
					$update = $wpdb->update($wpdb->postmeta, array('meta_value' => addslashes(serialize($certification))), array('meta_id' => $meta_id));
				} else {
					$update = add_post_meta($profile_id, 'certification', addslashes(serialize($certification)));
				}
				if ($update === false) {
					wp_send_json(array(
						'success' => false,
		...
		if (isset($request['education']) && !empty($request['education']) && is_array($request['education'])) {
			...
			if (!empty($education['title']) && !empty($education['subtitle'])) {
				if (!empty($education['id'])) {
					...
					unset($education['id']);
					global $wpdb;
					$update = $wpdb->update($wpdb->postmeta, array('meta_value' => addslashes(serialize($education))), array('meta_id' => $meta_id));
				} else {
					$update = add_post_meta($profile_id, 'education', addslashes(serialize($education)));
				}
				if ($update === false) {
					wp_send_json(array(
						'success' => false,
