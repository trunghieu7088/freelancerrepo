(function ($, Models, Collections, Views) {
  $(document).ready(function () {
    if ($("div.workplace-details").length > 0) {
      ...
      MessageItem = Views.PostItem.extend({
        ...
        onItemRendered: function () {
          ...
          // after render view
          view.$el
            .attr("id", "comment-" + this.model.get("comment_ID"))
            .attr("data-commentTime", this.model.get("comment_date_gmt"));
      ...
      // view control file upload for project files
            ...
            success: function (res) {
              ...
              if (res.success) {
                ...
                var template =
                  '<li class="attachment-' +
                  res.attachment.ID +
                  ' att-workspace-item-js">' +
                  "<p>" +
                  res.attachment.post_title +
                  "<span>" +
                  '<a href="' +
                  res.attachment.guid +
                  '" target="_blank" download><i class="fa fa-cloud-download" aria-hidden="true"></i></a>' +
                  '<a href="#" data-post-id="' +
                  res.attachment.ID +
                  '" data-project-id="' +
                  res.attachment.project_id +
                  '" data-file-name="' +
                  res.attachment.post_title +
                  '" class="delete-attach-file"><i class="fa fa-times" aria-hidden="true" data-post-id="' +
                  res.attachment.ID +
                  '" data-project-id="' +
                  res.attachment.project_id +
                  '" data-file-name="' +
                ...
                if (ae_globals.ae_is_mobile == "1") {
                  var template =
                    ...
                    res.attachment.ID +
                    '">' +
                    '<span class="file-attack-name"><a href="' +
                    res.attachment.guid +
                    '" target="_Blank">' +
                    res.attachment.post_title +
                    "</a></span>" +
                    '<span class="file-attack-time">' +
                    res.attachment.post_date +
                    "</span>" +
                    '<a href="#" data-post-id="' +
                    res.attachment.ID +
                    ...
                    '" data-file-name="' +
                    res.attachment.post_title +
                    '" class="delete-attach-file"><i class="fa fa-times pull-right" aria-hidden="true"></i></a>' +
                    "</li>";
                }
        ...
        removeFile: function (e) {
          ...
          for (i = 0; i < this.docs_uploader.controller.files.length; i++) {
            if (this.docs_uploader.controller.files[i].id === fileID) {
              this.docs_uploader.controller.removeFile(
                this.docs_uploader.controller.files[i]
              );
            }
          }
        },
      ...
      // view control file upload for chat area
      Views.UploaderChatFile = Backbone.View.extend({
        ...
        initialize: function (options) {
          ...
          this.docs_uploader = new AE.Views.File_Uploader({
            ...
            cbAdded: function (up, files) {
              var $file_list = view.$(".upload_file_file_list"),
                i;
              // Check if the size of the queue is over MAX_FILE_COUNT
              if (up.files.length > view.docs_uploader.MAX_FILE_COUNT) {
                // Removing the extra files
            ...
            success: function (res) {
              ...
              if (res.success) {
                ...
                if (
                  res.attachment.file_type == "png" ||
                  res.attachment.file_type == "jpg" ||
                  res.attachment.file_type == "jpeg" ||
                  res.attachment.file_type == "gif"
                ) {
                  icon = '<i class="fa fa-file-image-o"></i>';
                }
                var template =
                  '<li id="comment-' +
                  res.attachment.comment_id +
                  '">' +
                  '<span class="message-avatar">' +
                  res.attachment.avatar +
                  "</span>" +
        ...
        fileTemplate: _.template(
          '<li id="{{=id}}"><span class="file-name" >{{=filename }}</span><a href="#"><i class="fa fa-times removeFile"></i></a></li>'
        ),
      });
      ...
      Views.WorkPlaces = Backbone.View.extend({
        ...
        initialize: function (options) {
          ...
          if ($(".message-container").find(".postdata").length > 0) {
            var postsdata = JSON.parse(
              $(".message-container").find(".postdata").html()
            );
          ...
          // init block control for the list
          this.blockCT = new Views.BlockControl({
            collection: view.messages,
            el: $(".message-container"),
          });

          // init upload file control for project files
          this.docs_uploader = {};
          ...
          // init upload file in chat area
          this.chatfilecontroller = new Views.UploaderChatFile({
          ...
          // init heartbeat for real time message
          this.liveShowMsg();
          ...
          // Fetch changelog
          AE.pubsub.on("ae:addChangelog", this.renderChangelog, this);
        },
        renderMessages: function (result) {
          var view = this;
          if (result.success && result.comments && result.comments.length > 0) {
            result.comments.forEach(view.renderMessage, view);
          }
          $(".list-chat-work-place-wrap").mCustomScrollbar(
            "scrollTo",
            "bottom"
        ...
        renderMessage: function (msg) {
          ...
          if (
            !(
              view.listMessages.$el.find("#comment-" + msg.comment_ID).length >
              0
            )
          ) {
            if (msg.isAttach == "" && msg.template_file == "") {
              const partnerClass =
                ae_globals.user_ID != msg.user_id ? "partner-message" : "";

              if (msg.changed_milestone_id && msg.changed_milestone_id != "") {
                view.renderChangelog(msg);
              } else {
                view.listMessages.$el.append(
                  '<li class="' +
                    partnerClass +
                    '" id="comment-' +
                    msg.comment_ID +
                    '" data-commentTime="' +
                    msg.comment_date_gmt +
                    '">' +
                    template(msg) +
                    "</li>"
                );
            ...
            // comment out this code because when new_message ever = true???
            // if (typeof new_message != undefined && new_message == true) {
            //   $("#workspace_files_list").load(
            //     document.URL + " #workspace_files_list"
            //   );
            // }

            if (msg.isAttach != undefined && msg.isAttach == "isAttach") {
              // console.log("this should insert the attactment file");
              $(".upload_file_file_list").append(msg.template_file); // 1.8.16
              // comment this line because it auto append duplicate item in the  frame chat.
              // - anhcv turned back on
            }
          }

          if (typeof msg.remove_file != undefined) {
            // remove item attachment in chat area
            var item = ".attachment-" + msg.remove_file;
            $(item).remove();
          }
        },
        renderChangelog: function (msg) {
          let view = this;
          const partnerClass =
            ae_globals.user_ID != msg.user_id ? "partner-message" : "";
          view.listMessages.$el.append(
        ...
        sendMessage: function (target) {
          ...
          message.save("", "", {
            beforeSend: function () {
              view.$("textarea").attr("disabled", "disabled");
            ...
            success: function (result, res, xhr) {
              ...
              view.$("textarea").removeAttr("disabled");
              view.docs_uploader.controller.splice();
              view.docs_uploader.controller.refresh();
              if (res.success) {
                if ($(".textarea.content-chat").find($(".message-none"))) {
                  $(".message-none").remove();
                  if ($("#message-time-today").length <= 0) {
                    $(".list-chat-work-place").append(
                      '<li class="message-time" id="message-time-today">Today</li>'
                    );
                  }
                }
        ...
        liveShowMsg: function () {
          var view = this,
            $query_args = $("#workspace_query_args");

          // init heartbeat, send the 1st data (project post id) to server
          $(document).on("heartbeat-send", function (e, data) {
            if ($query_args.length > 0) {
              var qr = JSON.parse($query_args.html());
              const latestReadTime = view.getLastestReadTime();
              if (typeof qr["post_id"] !== "undefined") {
                data["project_id"] = qr["post_id"];
                data["latestReadTime"] = latestReadTime;
              }
            }
          });
          ...
          $(document).on("heartbeat-tick", function (event, data) {
            if (
              ...
              view.renderMessages(data);
            }
          });
        },
        getLastestReadTime: () => {
          const partnerMsgs = $("#workspace-conversation li.partner-message");
          return partnerMsgs.length > 0
            ? partnerMsgs.last().attr("data-commentTime")
            : 0;
        },
...
(function ($, Models, Collections, Views) {
  $(document).ready(function () {
    if ($("div.report-details").length > 0) {
      ...
      Views.ReportPlaces = Backbone.View.extend({
        ...
        initialize: function (options) {
          ...
          if ($(".report-container").find(".postdata").length > 0) {
            var postsdata = JSON.parse(
              $(".report-container").find(".postdata").html()
            );
          ...
          this.docs_uploader = this.filecontroller.docs_uploader;
          $(".content-require-project-report .workplace-title-wrap").on(
            "click",
            function () {
