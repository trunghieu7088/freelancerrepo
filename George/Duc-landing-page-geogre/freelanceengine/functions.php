...
class ET_FreelanceEngine extends AE_Base
{
	...
	function restrict_pages()
	{
		...
		if (is_page_template('page-list-notification.php') && !is_user_logged_in()) {
			wp_redirect(et_get_page_link('login', array('ae_redirect_url' => urlencode(et_get_page_link('list-notification', false)))));
			exit();
		}
	 ...
	 * @author Dakachi
	 */
	public function order_terms($orderby, $args, $taxonomies = array())
	{
	...
	function filter_link_redirect_register($result)
	{
		...
		if ($role == 'employer') {
			// if( ae_get_option('user_confirm') ) $redirect_url = et_get_page_link( 'profile' ); //AE_Users::is_activate( $result->ID )
	...
	function on_add_scripts()
	{
		...
		*/
		...
		if (is_active_sidebar('fre-footer-1')) {
		...
		if (is_page_template('page-my-project.php') || is_singular(PROJECT)) {
			$this->add_script(
				'my-project-page',
				get_template_directory_uri() . '/assets/js/my-project.js',
				array(
					'jquery',
					'underscore',
					'backbone',
	...
	function on_add_styles()
	{
		...
		//$this->add_style( 'scroll-bar', get_template_directory_uri() . '/assets/css/jquery.mCustomScrollbar.css', array(), ET_VERSION );
		//     return;
		// }
		// theme custom.css
		$this->add_style('main-style', get_template_directory_uri() . '/assets/css/styles.css', array(), ET_VERSION);
		// theme custom.css
		$this->add_style('custom', get_template_directory_uri() . '/css/custom.css', array(), ET_VERSION);

		$this->add_style('scroll-bar', get_template_directory_uri() . '/assets/css/jquery.mCustomScrollbar.css', ET_VERSION);


		// style.css
		$this->add_style('freelanceengine-style', get_stylesheet_uri(),  array(), ET_VERSION);
		//datepicker
		$this->add_style('datepicker-css', get_template_directory_uri() . '/assets/css/bootstrap-datetimepicker.min.css', array(), ET_VERSION);
	}
...
if (!function_exists('fre_comment_reply_link')) {
	function fre_comment_reply_link($string, $args, $comment)
	{
		if (get_option('comment_registration') && !is_user_logged_in()) {
			...
			$string = $args['before'];
			$string .= sprintf(
				'<a rel="nofollow" href="#" data-toggle="modal" class="comment-reply-login login-btn">%s</a>',
				$args['login_text']
			);
			$string .= $args['after'];
		}
	...
	if (!$check_fre_updated_bids) {
		...
		foreach ($list_users as $key => $user) {
			...
			$meta_id        = $wpdb->get_var($wpdb->prepare(
				"SELECT umeta_id FROM {$wpdb->usermeta} WHERE meta_key = 'credit_number' AND user_id = %s",
				$user->ID
			));
...
function et_debug_check()
{
	$log_path 	= WP_CONTENT_DIR . '/fre_track_payment.css';
	$log_url 	= home_url() . '/wp-content/fre_track_payment.css';
	if (strpos(home_url(), "enginethemes.com") !== false) {
		$log_url 	= get_track_directory('url');
		$log_path 	= get_track_directory('path');
	}

	echo '<div class="et-debug"><a target="_blank" href="' . $log_url . '">Track File</a> ';
	echo ' <a target="_blank" href="' . get_log_file('url') . '"> Log File </a>';
	echo ' <a href="' . home_url('?write=ok') . '"> Test Write </a>';
	echo ' <a href="' . home_url('?deltrack=1') . '"> Delete All </a></div>';

	$deltrack = isset($_GET['deltrack']) ? $_GET['deltrack'] : 0;
	$write = isset($_GET['write']) ? $_GET['write'] : 0;
	if ($deltrack) {
		unlink($log_path);
		unlink(get_log_file());
		echo 'Del track path done';
	} else if ($write == 'ok') {
		et_track_payment('TEST TRACK FILE OK. Allow overwrite this file.');
		et_log('TEST LOG FILE OK. Allow overwrite this file.');
	}
	?>
	<style type="text/css">
		div.et-debug {
...
if (ET_TRACK_PAYMENT && ET_LOG) {
	add_action('wp_footer', 'et_debug_check', 99);
}

function fre_credit_admin_notice()
{
	if (version_compare(ET_VERSION, '1.8.16')  >= 0 && is_plugin_active('fre-credit/fre-credit.php')) { ?>
		<div class="notice notice-success is-dismissible" style="background-color: red;">
			<h5 style="color: white; font-size: 18px;"><?php _e('Since  FreelanceEngine version 1.8.16. Fre Credit extension was moved to core theme so please disable/remove this plugin to avoid conflict issues may causes!', ET_DOMAIN); ?></h5>
		</div> <?php deactivate_plugins('fre-credit/fre-credit.php');
			}
		}
		// add_action( 'admin_notices', 'fre_credit_admin_notice' );


		function show_process_escrow_order($order_id, $total, $permalink)
		{
		...
		<div class="fre-page-section">
			<div class="container">
				<div class="page-purchase-package-wrap page-process-payment.php">
