...
class AE_Mailing extends AE_Base {
	...
	function forgot_mail( $user_id, $key ) {
		...
		$activate_url = '<a href="' . $activate_url . '">' . __( "Reset Password", ET_DOMAIN ) . '</a>';
		$message      = str_ireplace( '[activate_url]', $activate_url, $message ); // keep it for older content.
		$message      = str_ireplace( '[resetpass_url]', $activate_url, $message );
		...
		$subject = sprintf( __( '[%s] Reset Your Password', ET_DOMAIN ), $blogname );
	...
	function request_confirm_mail( $user_id ) {
		...
		$message = ae_get_option( 'confirm_mail_template' );
		// }
		$result =  $this->wp_mail( $user_email, $subject, $message, array(
			'user_id' => $user_id
		) );
		et_log('Send confirm_mail Result: ');
		et_log($result);
		return $result;
	}
	...
	/**
	 * send receipt email to buyer after submit a payment successful payment email
	 *
	...
	public function send_receipt( $user_id, $order ) {
		...
		$attachs_check = array('type' => 'invoice','orders' => $order['ID'],'payer' => $user) );
	}
	 ...
	 * @since 1.0
	 */
	public function wp_mail( $to, $subject, $content, $filter = array(), $headers = '', $attachs_check = array() ) {
		if ( $headers == '' ) {
		...
		$full_content 	= $this->get_mail_header() . $content . $this->get_mail_footer();
		$attachments 	= apply_filters('fre_email_attachments',array(), $attachs_check);
		...
		$a = wp_mail( $to, $subject, $full_content, $headers , $attachments );
	...
	public function et_mail( $to, $subject, $content, $filter = array(), $headers = '', $attachments = array() ) {
		if ( $headers == '' ) {
	...
	function filter_post_placeholder( $content, $post_id = '' ) {
		...
		$title = apply_filters( 'fre_title', $post->post_title );
