		...
		if ( ! empty( $this->pages ) ) {
			...
			foreach ( $this->pages as $key => $page ) {
				...
				if ( isset( $args['icon_class'] ) ) {
					...
					$custom_icon = isset($args['custom_icon']) ? $args['custom_icon'] : '';
					echo '<li>';
					echo '<a class="engine-menu ' . $active . '" href="?page=' . $args['slug'] . '">';
					if($custom_icon){
                        echo '   <div  class="custom-icon">'.$custom_icon.'</div>';
					} else {
						echo '<div data-icon="' . $args['icon'] . '" class="icon engine-menu-icon icon-' . $args['icon'] . '"></div>';
					}

                    echo '<div class="">' . $args['menu_title'] . '</div> </a></li>';
				}
				$active = '';
			}
			echo '</ul>';
...
class AE_Menu extends AE_Page {
	...
	function action_sync() {
		...
		$request 	= $_REQUEST;
		$name    	= $request['name'];
		$set_value 	= isset($request['value']) ? $request['value'] : '';
		$value 		= array();
		if ( is_string( $set_value ) ) {
			$request['value'] = stripslashes( $set_value );
		}
