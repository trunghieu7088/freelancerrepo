...
class AE_Posts {
	...
	public function convert( $post_data, $thumbnail = 'medium_post_thumbnail', $excerpt = true, $singular = false ) {
		...
		if ( has_post_thumbnail( $result['ID'] ) ) {
			...
			$result['the_post_thumnail'] = isset($feature_image[0] ) ? $feature_image[0] : 0;
		} else {
	...
	function filter_content( $content ) {
		...
		//$content = str_replace( '<a', '<a target="_blank" rel="nofollow noopener noreferrer"', $content ); // replace  in v1.8.21
	...
	/** dublicate sync method default to specific. easy track method process
	 * sync request from client,
	 * request should have attribute method to specify which action want to do
	 *
	 * @param array $request
