...
class AE_Users {
	...
	public function update( $user_data ) {
		...
		if ( isset( $user_data['do'] ) ) {
			switch ( $user_data['do'] ) {
				...
				case 'resend_confirm':
					$result->msg = __( "Email has been sent. Please check your email to confirm!", ET_DOMAIN );
					break;
				case 'ban':
	...
	public function login( $user_data ) {
		...
		if ( $result && ! is_wp_error( $result ) ) {
			...
			wp_set_auth_cookie($result->ID); // in version  1.8.15 - fix sync login with  back-end.
	...
	public function sync( $request ) {
		...
		switch ( $method ) {
			...
			case 'read':
				...
				} elseif ( 'resend_confirm' === $do ) {
					$result = $this->resend_confirm( $request );
				} elseif ( 'inbox' === $do ) {
	...
	function resend_confirm(){
		$resp = array(
			'success' 	=> true,
			'msg' 		=> __('Your confirm link has been sent. Please check email and confirm your email address.',ET_DOMAIN) );
		wp_send_json($resp);
	}
	/**
	 * user confirm email
	...
	public static function confirm( $key ) {
        ...
        if($user) {
			...
			// user had activated
			if ( self::is_activate( $user_id ) ) {
				return false;
			}

			$de_confirm = update_user_meta( $user_id, 'register_status', '' );
