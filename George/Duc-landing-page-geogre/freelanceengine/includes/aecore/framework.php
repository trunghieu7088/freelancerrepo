...
class AppEngine extends AE_Base {
	...
	public function print_scripts() {
		...
		$variable                     = array(
			...
			'homeURL'              => home_url(),
			//'purchase_bid'     	   => apply_filters('url_page_bid',et_get_page_link( 'upgrade-account' ) ),
			'url_purchase_bid'     => apply_filters('url_purchase_bid',et_get_page_link( 'upgrade-account' ) ), // 1.8.17
		...
		$variable['is_customize_preview'] 	= is_customize_preview();
		$variable['min_length'] 			= MINLENGTH_CONTENT;
            ...
            (function ($) {
                if (typeof $.validator !== 'undefined') {
                    $.extend($.validator.messages, {
                        ...
                        freMinLength: '<?php _e( "Please enter at least {0} characters.", ET_DOMAIN ); ?>',
		...
		if ( ! is_admin() ) {
			...
			if ( isset( $_GET['act'] ) && $_GET['act'] == "confirm" && $_GET['key'] ) {
				...
				if ( $user_id ) {
					...
					$redirect_url = ae_redirect_url_after_confirm();
	...
	function notify_admin( $payment_return, $data ) {
		...
		et_track_payment('notify_admin ==> send_mail&notification hook.');
		do_action( 'ae_notify_admin', $data );
		if ( ! isset( $data['ad_id'] ) || empty( $data['ad_id'] ) ) {
			return false;
		}
		if ( ! isset($payment_return['ACK'])  || ! $payment_return['ACK'] ) {
			// new to check carefully this. Update=> move to the top this condition.  v1.8.19
			// revert to botton default
			et_track_payment('ACK Fail => NO SEND Mail. NO NOTIFICATION.');
			return;
		}
		et_track_payment('mailing=>new_post_alert.');
		$this->mail = AE_Mailing::get_instance();
