...
function ae_currency_sign($echo = true)
{
	...
	$icon 		= isset($options->currency['icon']) ? $options->currency['icon'] : '$';
	if ($echo) {
		echo $icon;
	} else {
...
function ae_currency_code($echo = true)
{
	...
	// get option instance to retrieve option value
	$options = AE_Options::get_instance();
	$code  = isset($options->currency['code']) ? $options->currency['code'] : 'USD';
	if ($echo) // echo code
	{
		echo $code;
	} else // return code
...
function ae_currency_align($echo = true)
{
	...
	// get option instance to retrieve option value
	$options = AE_Options::get_instance();
	$align  = isset($options->currency['align']) ? $options->currency['align'] : 1;
	if ($echo) {
		echo $align;
	} else {
		return $align;
...
function ae_process_payment($payment_type, $data)
{
	...
	et_track_payment('method: ae_process_payment ==> payment_type==' . $payment_type);
	if ($payment_type) {
		...
		// verify ipn response from server.
		et_track_payment('start check IPN response');
		$payment_return = $order->accept($visitor); // ex: ->ET_PaypalVisitor ->do_checkout  [kiểm tra 1 order verfy từ server ở đây.]
		$data['order']        = $order;
		...
		// publish project/ ad in this hook
 ...
 * @copyright enginethemes.com team
 */
function et_get_page_link($slug, $params = array(), $create = false)
{
	$page_args = array(
		'post_title'   => '',
	...
	$page_args['post_title'] = $slug;

	$opt_link_name = 'page-' . $slug . '.php';
	$link 	= get_option($opt_link_name, '');
	if ($link) {
		$return = $link;
		if ($params) $return = add_query_arg($params, $link);
		return apply_filters('et_get_page_link', $return, $slug, $params);
	}
	// find post template
	$pages = get_pages(array(
		'meta_key'    => '_wp_page_template',
		'meta_value'  => $opt_link_name, //'page-' . $slug . '.php',
		'numberposts' => 1
	));
	$id = 0;
	if ($pages) {
	...
	update_option($opt_link_name, $return);
...
function et_get_page_linkv0($slug, $params = array(), $create = false)
{
	$page_args = array(
		'post_title'   => '',
		'post_content' => __('Please fill out the form below ', ET_DOMAIN),
	 ...
	 */
	$opt_page_name = 'page-' . $page_type . '.php';
	$link 	= get_option($opt_page_name, '');
...
function is_social_connect_page()
{
	...
	$connect_page_id 	= ae_get_option('social_connect');
	$social_connect_page = get_permalink($connect_page_id);
	$social_connect_page = str_replace('www.', '', $social_connect_page);

	// check the current page slug match with social connect setting
	if ($social_connect_page == $connect_page_id) {
		return true;
	}
	$site_url = home_url('/');
	...
	if (!empty($current_page_slug) && strpos($social_connect_page, $current_page_slug)) {
		return true;
	}
	if ($page_connect_id) {
...
if (!function_exists('ae_get_social_login_user_roles_default')) {
	function ae_get_social_login_user_roles_default()
	{
		...
		return apply_filters('ae_social_login_user_roles_default', $roles);
	}
}
...
/**
 */
if (!function_exists('ae_redirect_url_after_confirm')) {
	function ae_redirect_url_after_confirm()
	{
		return home_url();
	}
}

function ae_enable($opt_name = '')
{
	return ae_get_option($opt_name, true);
}
