...
abstract class  ET_PaymentVisitor {
	...
	function add_return_param() {
		...
		et_track_payment('PaymentVisitor Return URL: '.$link); // check http or https. make sure the same with home page.
...
class ET_PaypalVisitor extends ET_PaymentVisitor {
	...
	function setup_checkout( ET_Order $order ) {
		...
		et_track_payment('returnURL: '.$returnURL);
		$notify_url = urlencode( add_query_arg( 'paypalListener', 'paypal_appengine_IPN', trailingslashit( home_url() ) ) );
		$notifyURL = '&notify_url=' . $notify_url ;
		et_track_payment('notifyURL: '.$notify_url);
	 ...
	 * @author Dakachi
	 */
	function do_checkout( ET_Order $order ) { // Verify paypal IPN response
		et_track_payment('Method: ET_PaypalVisitor->do_checkout');
		et_track_payment('verify PayPal IPN response');
	 ...
	 * @since 1.2
	 * @author Dakachi
	 * similar: check_ipn_response
	 */
	function do_checkout_get_back( ET_Order $order, $paymentStatus ) {
		et_track_payment('Method: ET_PaypalVisitor->do_checkout_get_back()');
		...
		if ( isset( $_REQUEST['st'] ) && ( $_REQUEST['st'] == 'Completed' || $_REQUEST['st'] == 'Pending' ) ) {
			...
			if ( $_REQUEST['amt'] == $order['total'] && $_REQUEST['cc'] == $order['currencyCodeType'] ) {
				et_track_payment('Valiate order success(mc_ross + currency  code).');
				/**
				 * check status update order
				 */
				if ( $_REQUEST['st'] == 'Completed' ) {
					et_track_payment('Set payment to publish.');
					$order_pay->set_status( 'publish' );
				} else {
					et_track_payment('Set paymetn to pending.');
		...
		if ( isset( $_POST['payment_status'] ) && ( $_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Pending' ) ) {
			...
			et_track_payment('check_fraud. payment_status == :'.$_POST['payment_status']);
			...
			if ( $mc_gross == $order['total'] && $mc_currency == $order['currencyCodeType']
			     &&  $business == trim( $api['api_username']  )
			) {
				et_track_payment('check amount(total), currency OK. => Approve order here 2.');
		...
		et_track_payment('check_fraud PAYPAL IPN FAIL :');
		// If false
		return array(
			'ACK'            => false,
			'payment'        => 'simplePaypal',
...
class ET_GoogleVisitor extends ET_PaymentVisitor {
	...
	function do_checkout( ET_Order $order ) { // Verify Google Checkout IPN response
		$order_pay = clone $order;
		$order     = $order_pay->generate_data_to_pay();
...
class ET_CashVisitor extends ET_PaymentVisitor {
	...
	function do_checkout( ET_Order $order ) { // verify Cash response
		$order_pay = clone $order;
		$order     = $order_pay->generate_data_to_pay();
		if ( isset( $order['ID'] ) ) {
			$cash = new ET_Cash ( $this->_settings );
...
class ET_AuthorizeVisitor extends ET_PaymentVisitor {
	...
	function do_checkout( ET_Order $order ) { // Verify Authorize Response
