...
class ET_FaceAuth extends ET_SocialAuth {
	...
	public function __construct() {
		...
		$this->fb_app_id         = ae_get_option( 'et_facebook_key' );
		$this->fb_secret_key     = ae_get_option( 'et_facebook_secret_key' );
		$this->fb_app_id 		 = trim($this->fb_app_id);
		$this->fb_secret_key  	 = trim($this->fb_secret_key);
	...
	public function auth_facebook() {
		try {
			...
			$fb_exchange_token .= '?grant_type='.$grant_type.'&';
			...
			$body = wp_remote_retrieve_body($fb_app_token);

			$response  = json_decode( $body );

			if($response->error){

				$msg_error = $response->error->message;
			...
			$data = $_POST['content'];
			// content[name]: Dần Nguyễn Gia
			// content[email]: danng@gmail.vn
			// content[id]: 1475616179291416
			// fb_token:
			...
			if ( $user ) {
				...
				if(NEW_SOCIAL){ // auto insert new wp_user
					...
					$user_meta = array(
						'et_facebook_id' => $data['id'],
						//'et_auth_type' => 'facebook',
						'et_avatar' => $avatars,
