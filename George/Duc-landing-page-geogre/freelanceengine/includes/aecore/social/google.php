		...
		if ( ( isset( $_REQUEST['code'] ) && ! empty( $_REQUEST['code'] ) ) && ( isset( $_REQUEST['state'] ) || $_REQUEST['state'] == $this->state ) ) {
			try {
				...
				if ( $this->get_user( $userinfor->id ) ) {
					et_log($userinfor->email.' - this google register - exists');
					...
					et_log($userinfor->email.' - this google did not register - add new');
					/*[id] => 107663926879700194339
				    [email] => abc@gmail.com
				    [verified_email] => 1
				    [picture] => https://lh3.googleusercontent.com/a-/AOh14GiJW7xdzIUJeLWDr63OwKCxlfGVzmPVyNQQxWzh2A
				    */
					// avatar
					...
					if( NEW_SOCIAL ){ // auto set user_login name + display name;
						fre_insert_google_user($userinfor, $avatar);
