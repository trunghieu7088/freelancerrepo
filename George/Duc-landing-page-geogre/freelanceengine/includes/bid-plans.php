...
function fre_button_bid( $project_id ) {
	...
	if ( ! $profile || ! is_numeric( $profile_id ) ) {
		...
		echo '<a href="#" class="fre-normal-btn purchase-bid-btn primary-bg-color" btnbid1 data-project-id="' . $project_id . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
	} else {
		// Check invite to bid
		if ( ae_get_option( 'invited_to_bid' ) && ! fre_check_invited( $user_ID, $project_id ) ) {
			//echo '<a class="fre-normal-btn ' . $class . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
			echo '<a href="#" class="fre-normal-btn purchase-bid-btn primary-bg-color" btnbid2 data-project-id="' . $project_id . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
		} else {
			// disable option pay to bid
			if ( ! can_user_bid( $user_ID ) ) {
				echo '<a href="#" class="fre-normal-btn purchase-bid-btn primary-bg-color"  btnbid3 data-project-id="' . $project_id . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
			} else {

				echo '<a href="#" class="fre-normal-btn primary-bg-color btnbid4" btnbid4 data-toggle="modal" data-target="' . $target . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
			}
		}
	}
}