 ...
 * Time: 3:51 PM
 */
if( ! class_exists('FRE_Credit_Employer') ):
class FRE_Credit_Employer extends FRE_Credit_Users{
    ...
    public function fre_submit_project_payment($order){
        et_track_payment('fre_submit_project_payment');
        et_track_payment('update balance of employer when pay package to post_project');
        global $user_ID;
    ...
    public function fre_credit_setup_payment($response, $paymentType, $order){
      // fre_credit_fix or
        et_track_payment('fre_credit_setup_payment ==> Update balance + publish project. Input paymentType = '.$paymentType);
    ...
    */
    ...
    /**
      ...
      * Gọi method này lúc mua bid or post_project bằng fre_creidt. (not sure - cần check lại)
      *
      * @param void
    ...
    public function fre_credit_process_payment( $payment_return, $data ){
        ...
        $order = $data['order'];
        et_track_payment('fre_credit_process_payment of class-credit-employer.phpp => input payment_type=='.$payment_type); //input payment_type==bitcoin
        if( $payment_type == 'frecredit') {
            if( isset($_REQUEST['token']) &&  $_REQUEST['token'] == $order->get_payment_code() ) {
                ...
                et_track_payment('fre_credit_process_payment ==> update_status order');
                $order->set_status ('publish');
                et_track_payment('call method: $order->update_order().');
                $order->update_order();
