 ...
 * Time: 1:42 PM
 */
if( ! class_exists('FRE_Credit_Escrow') ):
class FRE_Credit_Escrow extends AE_Base{
    ...
    public function init(){
        ...
        $this->add_action( 'fre_credit_finish_escrow', 'finishEscrow', 10, 2 ); // v1.8.16 change hook name fre_finish_escrow
        $this->add_filter( 'fre_process_escrow', 'processEscrow', 10, 3 );
        $this->add_action( 'ae_escrow_execute', 'executeEscrow', 10, 2 ); // disputed and freelancer win.
        //$this->add_action( 'ae_escrow_refund', 'refundEscrow', 10, 2 );
        $this->add_action( 'ae_credit_escrow_refund', 'refundEscrow', 10, 2 ); // v1.8.16 change hook name for credit_escrow system
    ...
    public function processEscrow( $payment_return, $payment_type, $data ){
        ...
        if($payment_type == 'frecredit' && isset($_REQUEST['success']) && $_REQUEST['success']){
            ...
            et_log('processEscrow:');
            if( ! isset($data['order_id']) ){
              et_log('exit_empty_order');
              return $payment_return;
            }
            $order_id       = isset($data['order_id']) ? $data['order_id'] : 0;
            $obj_order      = new AE_Order($order_id); // v1.8.19
            $order_array    = $obj_order->get_order_data(); // v1.8.19
            ...
            et_log('send_mail_done');
            et_track_payment('processEscrow::START ');
            $this->mail->new_payment_notification_mail($order_id);

            //update status order
            wp_update_post(array('ID' => $order_id, 'post_status' => 'publish'));
            $project_id = isset($data['ad_id']) ? $data['ad_id'] : 0;
            if( $project_id ){
                //update status project
                if( current_user_can(  'manage_options' ) || !ae_get_option('use_pending', false)){
                    et_track_payment('processEscrow: update project_status to publish( purchase package via credit). Ad_ID'.$data['ad_id']);
            ...
            if ( $order_id ) {
                ...
                if(isset($_REQUEST['packageType']) && $_REQUEST['packageType'] == '1'){
                    et_track_payment('Update UserPack_data');
                    AE_Payment::update_current_order( $payer_id, $sku, $order_id );
                    AE_Package::add_package_data($sku, $payer_id);
                    AE_Package::update_package_data($sku, $payer_id);
                } elseif( isset($_REQUEST['packageType']) && $_REQUEST['packageType'] == 'bid_plan'){

                    et_log('update_credit_number');
                    et_track_payment('after_fre_pay_to_bid');
                    /*
                    comment since 1.8.19
                    // Update credit number for user when freelance buy package bid by credit.
                    //$order_credit = get_post($data['order_id']); //comment in v1.8.19
                    // $meta         = get_post_meta($order_id, 'et_order_products', true);
                   ...
                   */
                   // Upgrade code
                    ...
                    if( $pack->et_number_posts > 0 ){
                        update_credit_number( $payer_id, $pack->et_number_posts );
                        // Use this hook to convert  a membership for freelancer if use frecrdit method.
                        do_action('after_fre_pay_to_bid',$payment_return,$data); // add in fre 1.8.16
                    }
                }
            }
            do_action('ae_member_process_order', $payer_id, $order_array); //
            $data['order'] = $order;
    ...
    /**
      * execute escrow after dispute and feelancer is winner.
      *
      * @param integer $project_id
      * @param integer $bid_id_accepted
      * @return void
