 ...
 * Time: 11:27 AM
 */
if( ! class_exists('FRE_Credit_Users') ):
class FRE_Credit_Users extends AE_Base{
    ...
    public function fre_credit_process_payment( $payment_return, $data ){
        ...
        et_track_payment('fre_credit_process_payment ==> start caculate and add credit. User ID: '.$user_ID.'. Order Id:'.$order_id);

        if ( ! isset($payment_return['ACK']) || !$payment_return['ACK']) {
            et_track_payment('fre_credit_process_payment ==> Exit ( ACK fail).Balance No change.');
            return false;
        }
        $paymnet_gw = strtolower($payment_return['payment']);


        $order_type = get_post_meta( $order_id,  'order_type',true );
        $order_p    = get_post($order_id);
        et_track_payment('paymnet_gw: '.$paymnet_gw.' Order_status:'.$order_p->post_status);
        if( $order_p->post_status !== 'draft' && ( $paymnet_gw == 'paypal' || $paymnet_gw == 'simplepaypal') ){
            et_track_payment('Exit. <<< This is specific flow. Was processed  in frev2_paypal_reponse().');
            return 0;
        }
        if( $order_type == 'fre_credit_fix') { // update for version 1.2.3
          et_track_payment('fre_credit_fix');
            $payment_status = isset($payment_return['payment_status']) ? $payment_return['payment_status'] : '';
            $bid_id = $data['ad_id'];
            // $deposit_info = fre_get_deposit_info($bid_id); // disable in 1.8.19
            ...
            if(  $payment_return['payment'] != 'cash' ){
               ...
               if($payment_status == 'Completed'){
                    et_track_payment('>>> Cal deposite method.');
          ...
          et_track_payment('not_fre_credit_fix');
          if( isset($payment_return['payment_status']) ){
              ...
              if($pack != false){
                  ...
                  if( $payment_return['payment_status'] == 'Completed' && $payment_return['payment'] != 'cash' ){
                      ...
                      if( isset( $pack->et_number_posts ) && (float)$pack->et_number_posts > 0 ){
                            et_track_payment('save history for this deposit only. Order ID: '.$order_pay['ID']);
                          $default['status'] = 'pending';
                          $history_id = FRE_Credit_History()->saveHistory($default);
    ...
    public function update_order_credit($order_id){
        et_track_payment('update_order_credit => Approve deposit in backend.');
        global $ae_post_factory;
        $history_obj    = $ae_post_factory->get('fre_credit_history');
        $order_status   = $_REQUEST['status'];
        $order_id       = $_REQUEST['ID'];

        $payment_gateway = get_post_meta($order_id,'et_order_gateway', true);
        if( $payment_gateway == 'cash' ){
            et_track_payment(' this is cash.');
            // processing in the method: fre_credit_cash_approved;
            return 0;
        }
        if($order_status == 'publish'){
            ...
            if(! $oder->post_parent ){
                et_track_payment('Create history for this transaction.');
                ...
                if( $order_type == 'fre_credit_fix') { // update for version 1.2.3
                    $number_credit = get_post_meta($order_id,'et_order_total', true);
                    $default['package_name'] = 'Deposit custom amout.';
                } else{

                    $packs  = AE_Package::get_instance();
                    $sku    = $order_data['payment_package'];
                    $pack   = $packs->get_pack($sku, 'fre_credit_plan');
                    $default['package_name'] = 'Deposit deposit via package '.$order_data['payment_package'];
                    $number_credit = max($pack->et_number_posts,0);
                }

                $default['amount'] = $number_credit; // amout in cents.
                $default['status'] = 'completed';
                $history_id = FRE_Credit_History()->saveHistory($default);
                if( is_wp_error($history_id)){
                    et_track_payment('save history_id fail');
                }
                wp_update_post(array('ID' => $oder_id,'post_parent' => $history_id) );
            } else {
            ...
            if($post_history->post_type == 'fre_credit_history'){
                ...
                $historyP = get_post($history_id);
                et_track_payment('history_title: '.$historyP->post_title);
                if($historyP->post_title == 'Deposited'){
                    // update deposit
                    et_track_payment('Aproved and update deposit for user ID '.$history->post_author.'. Add + '.$history->amount.' credits to ballance.');
                    $this->deposit($history->post_author, $history->amount);
                    update_post_meta($history_id, 'history_status', 'completed');
                } else {
                    et_track_payment('No update deposit.');
                }
            } else{
                et_track_payment('Faile. history_type: '.$post_history->post_type);
            }
   ...
   function buy_credit_order_infor($order_info){
      ...
      global $user_ID;
      $custom_amount = isset($_POST['custom_amount']) ? $_POST['custom_amount'] : 0;

      if( $order_info['post_type'] == 'fre_credit_fix') {
        if( ! $custom_amount ){
          //   base on the bid_id to caculate amount
          ...
          $order_info['et_price'] = ceil($custom_amount);
        }
      }
      return $order_info;
   }
    ...
    public function deposit($user_id, $number){
        ...
        et_track_payment('Deposit Credit for user ID: '.$user_id);
        et_track_payment('Current ballance: '.$user_wallet->balance);
        $user_wallet->balance = $number + $user_wallet->balance;
        et_track_payment('Add number credit: '.$number);
        et_track_payment('New balance: '.$user_wallet->balance );
    ...
    /**
      * approve cash
      * Keyword: approve_cash cash_approve
      * @param void
      * @return void
      * @since 1.0
      * @package FREELANCEENGINE
    ...
    public function fre_credit_cash_approved( $post_ID, $post ) {
        // move this process to update_order_credit method.
        global $user_ID;
        if( current_user_can('manage_options') ){
            if( $post->post_type == ET_ORDER && $post->post_status == 'publish' ){
