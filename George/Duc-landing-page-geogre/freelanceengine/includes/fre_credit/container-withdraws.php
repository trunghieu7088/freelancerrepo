...
if( !class_exists('AE_WithdrawList') ) :
/**
...
if(! class_exists('Fre_Credit_WithdrawAction') ):
    class Fre_Credit_WithdrawAction extends AE_PostAction
    {
        ...
        public function ae_withdraw_notify_item( $content, $notify, $type){
            $post_excerpt = str_replace('&amp;', '&', $notify->post_excerpt);

            parse_str($post_excerpt, $result);
            $admin = isset($result['admin']) ? $result['admin'] : FALSE;
...
endif; // end check class_exit;

if(! function_exists('fre_credit_admin_footer_function')):
    /**
     * add footer template
     *
     * @param void
...
if( ! function_exists('get_withdraws')):
    /**
     * get withdraws list
     *
     * @param array $args
