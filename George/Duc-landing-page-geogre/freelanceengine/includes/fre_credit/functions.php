<?php
/**
 * get current escrow system using in website.
  * @since 1.8.16
*/
function fre_get_escrow_using(){
    $use_escrow = ae_get_option( 'use_escrow', 0 );
    if( ! $use_escrow ){
        return false;
    }
    $escrow_system_using = ae_get_option('escrow_system_using', false); // credit_esscrow, stripe_escrow, paypal_adpative
    if( ! $escrow_system_using && ae_get_option('user_credit_system', false) ){
        // set default escrow is credit_escrow
        $escrow_system_using = 'credit_esscrow';
    }
    return $escrow_system_using;
...
if( !function_exists('fre_credit_deposit_page_link') ):
/**
  * deposit page link
  *
  * @param void
...
if( ! function_exists('fre_credit_get_admin_email') ):
/**
  * get admin email
  *
  * @param void
...
if ( ! function_exists('fre_credit_redirect') ):
/**
...
if(! function_exists('get_color_icon_transaction') ):
    /**
...
if( ! function_exists('get_list_credit_pending') ):
/**
...
if( ! function_exists('credit_get_total_withdraw') ):
...
if( !function_exists('credit_get_total_project_payment') ):
...
if( ! function_exists('credit_get_total_project_working') ):
...
if( ! function_exists('fre_credit_deposit_notify_buyer') ):
function fre_credit_deposit_notify_buyer( $content, $pack, $type ){
    ...
    if ( $type == 'fre_credit_plan' ) {
        ...
        $amout  = fre_price_format($pack->et_number_posts);
...
if( !function_exists('fre_mail_notify_admin_has_new_deposit') ) {
    /**
     * get email content
     *
