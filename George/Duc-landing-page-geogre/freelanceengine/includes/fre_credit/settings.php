<?php
if( ! function_exists('fre_credit_get_list_page') ):
  /**
   * Get lists page
   * @author ThanhTu
   * @since 1.0
...
if( !function_exists('fre_credit_menu_credit' ) ){
    function fre_credit_menu_credit( $pages ){
        ...
        $credit_plus_args   = apply_filters(
            'credit_plus_opts', array(
                'args'=>array(),
                'fields' => array(
                    array(
                        'id' => 'allow_custom_deposit',
                        'type' => 'switch',
                        'title' => __("Use credit system", ET_DOMAIN) ,
                        'name' => 'allow_custom_deposit',
                        'class' => '',
                        'default' => 1,
                    )
                )
        ...
        $credit_plus_mails = apply_filters('credit_plus_mails',array() );
        ...
        $credit_fields = apply_filters('credit_plus_mails', $credit_fields);
        $section = array(
            'args' => array(
...
if( ! function_exists('fre_credit_default_option') ):
  /**
...
if( !function_exists('fre_escrow_payment_gateway_credit_setting') ){
    ...
    * @author Jack Bui
    */
    function fre_escrow_payment_gateway_credit_setting($groups, $escrow_system_using){
	    $mail_super_admin           = get_option('admin_email');
        $enable_credit_system       = ae_get_option('user_credit_system', false);

        if( $enable_credit_system && $escrow_system_using === 'credit_escrow' ){
            $groups[] = array(
                'args' => array(
...
add_filter( 'fre_escrow_payment_gateway_settings', 'fre_escrow_payment_gateway_credit_setting', 10, 2 );
...
if ( ! function_exists('fre_credit_default_email') ):
/**
  * default email
  *
...
function fre_credit_default_email($mail_template){
    $mail_template['fre_credit_deposit_mail_template'] = "<p>Hi,</p>
                                    <p>User [display_name] has been deposited [number_credit] credits on your site,please check and confirm(if required) the payment.</p>
                                    <p>Sincerely,</p>
                                    <p>[blogname]</p>";
