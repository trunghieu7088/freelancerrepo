...
Class Fre_Mailing extends AE_Mailing {
	...
	function new_payment_notification_mail( $order_id ) {
		...
		if( $opt_send ){
			...
			if( !$order ){
				...
				$m_product = get_post_meta( $order_id, 'et_order_products', true );
				$product = current( $m_product );
	...
	function new_report( $project_id, $report ) {
		...
		$admin_template = str_replace( '[employer]', $employer, $admin_template );
	...
	function new_project_of_category( $project ) {
		...
		$tax_name = get_tax_name_matching_email();
		if(!empty($project->tax_input[$tax_name])){
			...
			foreach ( $project->tax_input[$tax_name] as $key => $value ) {
	...
	//do_action('sent_email_match_skills', $project);
}
