	...
	function convert($comment, $thumb = 'thumbnail', $merge_post = false, $merge_author = false)
	{
		...
		if (!empty($file_attach_arr) && is_array($file_attach_arr)) {
			...
			foreach ($attachment as $key => $value) {
				...
				echo '<li class="partner-message" id="comment-' . $comment->comment_ID . '" data-commentTime="' . $comment->comment_date_gmt . '">
...
class Fre_MessageAction extends AE_Base
{
	...
	function sendMessage()
	{
		...
		$comment = $this->comment->insert($_REQUEST); // this returns the comment ID when success or an wp-error when false
		...
		if (!is_wp_error($comment)) {
			...
			/**
			 * fire an action fre_send_message after sending message
			 *
			...
			// this action is currently used to launch the notification - anhcv

			// update the "newMsgTime" metadata to the created gmt date of the latest message
			// for the unread/read feature & realtime using heartbeat - anhcv
			$this->fre_update_newMsgTime($project, $comment->comment_date_gmt);
	...
	public function fre_send_data_to_heartbeat($response, $data)
	{
		if (isset($data['project_id'])) {
			...
			$post = get_post($data['project_id']);
			if ($post) {
				$project_obj             = $ae_post_factory->get(PROJECT);
				$project                 = $project_obj->convert($post);

				$newMsgTime = strtotime($this->fre_get_newMsgTime($project));

				// fetch the new message if there newMsgTime>latestReadTime
				// or when there is new milestone changelog
				if (
					("0" == $data['latestReadTime']) || ($newMsgTime > strtotime($data['latestReadTime']))
					||
					(isset($response['milestone_new_changelog']) && $response['milestone_new_changelog'] !== 0)
				) {
					$query = $this->get_query_args($project->ID);
					$review_object = $this->comment;
					if ("0" !== $data['latestReadTime']) {
						$query['date_query'] = [
							'column'	=> 'comment_date_gmt',
							'after'     => $data['latestReadTime'],
							'inclusive' => true,
						];
					}
					...
					$comments = $review_object->fetch($query);

					$response['success'] = true;
					$response['comments'] = array_reverse($comments['data']);
				} else {
	...
	public function get_query_args($project_id)
	{
	...
	/**
	 * update project meta data with newMsgTime for each role
	 * if current logged-in user is the project author (employer), update newMsgTime for the Freelancer
	 * otherwise, update newMsgTime for the employer
	 * @param object $project
	 * @param string $newMsgTime - the time string (comment_date_gmt) of the recently posted message
	 * @return void
	 * @author anhcv
	 * @since FrE 1.8.21
	 */
	public function fre_update_newMsgTime($project, $newMsgTime)
	{
		try {
			global $user_ID;
			if ($user_ID == $project->post_author) {
				update_post_meta($project->ID, 'fre_freelancer_newMsgTime', $newMsgTime);
			} else {
				update_post_meta($project->ID, 'fre_employer_newMsgTime', $newMsgTime);
			}
		} catch (Exception $e) {
			throw new Error("Invalid project or newMsgTime input. ", $e->getMessage());
		};
	}

	public function fre_get_newMsgTime($project)
	{
		global $user_ID;
		if ($user_ID == $project->post_author) {
			$newMsgTime = get_post_meta($project->ID, 'fre_employer_newMsgTime', true);
		} else {
			$newMsgTime = get_post_meta($project->ID, 'fre_freelancer_newMsgTime', true);
		}
	 ...
	 * @category void
	 * @author Tambh
	 * @deprecated use fre_update_newMsgTime instead
	 */
	 ...
	 * @category void
	 * @author Tambh
	 * @deprecated use fre_get_newMsgTime instead
	 */
