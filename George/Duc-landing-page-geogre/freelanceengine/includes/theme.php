...
if (!function_exists('is_use_credit_escrow')) {
    ...
    function is_use_credit_escrow()
    {
        ...
        if ($escrow_using == 'credit_escrow' || empty($escrow_using)) {
            return true;
        }
...
if (!function_exists('et_log')) {
    ...
    function et_log($input, $log_store = '')
    {
        if (ET_LOG) {
            $log_store = get_log_file('path');
            if (is_array($input) || is_object($input)) {
                error_log(date('Y-m-d H:i:s', current_time('timestamp', 0)) . ': ' . print_r($input, TRUE), 3, $log_store);
            } else {
...
function get_log_file($type = 'path')
{

    $upload_dir = wp_upload_dir();

    $upload_dir['path']        = $upload_dir['path'] . '/et_log.css';
    $upload_dir['url']         = $upload_dir['url'] . '/et_log.css';
    return $upload_dir[$type];
}

function get_track_directory($type = 'path')
{

    $upload_dir = wp_upload_dir();

    $upload_dir['path'] = $upload_dir['path'] . '/fre_track_payment.css';
    $upload_dir['url']     = $upload_dir['url'] .= '/fre_track_payment.css';
    return $upload_dir[$type];
}
function et_track_payment($input)
{
    ...
    if (ET_TRACK_PAYMENT) {
        $log_path = WP_CONTENT_DIR . '/fre_track_payment.css';

        if (strpos(home_url(), "enginethemes.com") !== false) {
            $log_path = get_track_directory();
        }
        if (is_array($input) || is_object($input)) {
            error_log(date('Y-m-d H:i:s', current_time('timestamp', 0)) . ': ' . print_r($input, TRUE), 3, $log_path);
        } else {
            error_log(date('Y-m-d H:i:s', current_time('timestamp', 0)) . ': ' . $input . "\n", 3, $log_path);
        }
    }
}
function is_post_project_free()
{
    $is_free = ae_get_option('disable_plan', false);
    return apply_filters('is_post_project_free', $is_free);
}
...
function fre_price_format($amount, $style = '<sup>')
{
    ...
    if ($align != "0") {
        ...
        return sprintf($format, $icon, number_format((float) $amount, $decimal, $decimal_point, $thousand_sep));
    } else {
        $format = '%s' . $format;

        return sprintf($format, number_format((float) $amount, $decimal, $decimal_point, $thousand_sep), $icon);
    }
}
...
function fre_number_format($amount, $echo = true)
{
    ...
    $thousand_sep  = (isset($number_format['thousand_sep']) && $number_format['thousand_sep']) ? $number_format['thousand_sep'] : get_theme_mod('et_thousand_sep', ',');
    if ($echo) {
        return number_format((float) $amount, $decimal, $decimal_point, $thousand_sep);
    } else {
        return number_format((float) $amount, $decimal, $decimal_point, $thousand_sep);
    }
}
                ...
                if (!is_wp_error($attach_id)) {
                    ...
                    try {
                        ...
                        if ($imgType == 'file') {
                            ...
                            // update new_message & newMsgTime for real-time order conversation
                            $project = get_post($project_id);
                            $comment = get_comment($comment_id);
                            $message_action = Fre_MessageAction::get_instance();
                            $message_action->fre_update_newMsgTime($project, $comment->comment_date_gmt);
                        }
...
/**
 * get availble bid of freelancer.
 * @keyword get_number_bid, fre_get_number_bid
 **/
function fre_get_total_bid($user_id = 0)
{
    ...
    $free_bid             = fre_get_free_bid_current_month($user_id);

    $number_credit         = get_user_credit_number($user_id); // pay_to_bid

    $subscription_bid     = apply_filters('get_subscription_number_bid', 0, $user_id); // in fre_membership if active

    return $free_bid + $number_credit + $subscription_bid;
}
                ...
                <div class="fre-work-package">
                    ...
                    if (!is_acti_fre_membership()) {
                        ...
                        if ($total_bids > 0) {
                            ...
                            if ($credits_pending > 0) {
                                printf(__('<p>You have <span class="number"><b>%d</b></span> available bid(s).</p>', ET_DOMAIN), $total_bids);
                                printf(__('<p><span class="number">%s</span> pending bid(s) are under admin review.</p>', ET_DOMAIN), $credits_pending);
                            } else {
                    ...
                    ?> <a class="fre-normal-btn-o" href="<?php echo et_get_page_link('upgrade-account'); ?>"><?php _e('Purchase more bids', ET_DOMAIN); ?></a><?php
                                                                                                                                                            } else {
                                                                                                                                                                do_action('overview_freelancer_bid_info', $total_bids);
                                                                                                                                                            } ?>
...
{ ?>
    <a class="emp-author-link" href="<?php echo get_author_posts_url($user_data->ID); ?>">
         ...
         * @since: 1.8.9
         */
        function fre_get_free_bid_current_month($user_id = 0)
        {
            global $user_ID;
            if (!$user_id)
                $user_id = $user_ID;
            ...
            return (int) ae_get_option('fre_free_bid', 0) - $bidded_this_month;
        }
        ...
        if (!function_exists('get_tax_name_matching_email')) {
            function get_tax_name_matching_email()
            {
                return 'skill';
            }
        }
        ...
        /**
         * Main Header Menu of the FreelanceEngine theme.
         * @since 1.8.15
         * @author: danng
         */
        function fre_header_menu()
        {
            if (!function_exists('fre_custom_header_menu')) {
                get_template_part('template/header', 'menu');
            } else {
                fre_custom_header_menu();
            }
        }
        ...
        /**
         * disable this filter in version v1.8.20 1.8.20
         * cause a conflict and can not save textarea field(v1.8.20)
         **/
        add_filter('teeny_mce_before_init', 'fre_set_minlength_content');
        function fre_set_minlength_content($initArray)
        {
            if (!is_page_template('page-submit-project.php')) {
                return $initArray;
            }
            if (!MINLENGTH_CONTENT) return $initArray;

            $initArray['setup'] = <<<JS
    [function(ed) {
        ed.onKeyUp.add(function(ed, e) {
            var content = ed.getContent();
            jQuery("#minlength_content").val(content);
            var leng = content.length;

            var wrap = jQuery("#minlength_content").closest('div.fre-input-field');
            var hasdiv = wrap.find(".message");
            console.log(hasdiv);
            var t = jQuery.validator.messages;
            var message = t.freMinLength;
            console.log(message);
            if( hasdiv.length < 1 ){
                wrap.append('<div class="message"></div>');
            }
               if(leng < ae_globals.min_length){
               hasdiv.show();
               hasdiv.html( message.replace('{0}',ae_globals.min_length ) );
           } else {
                   hasdiv.hide();
           }
    ...
    JS;
        ...
        /**
         * Quote icon in the list testimonial of home page.
         * @since: 1.8.18
         **/


        function fre_quote_svg_icon()
        { ?>
    <svg version="1.1" id="Forma_1_1_" xmlns:x="&ns_extend;" xmlns:i="&ns_ai;" xmlns:graph="&ns_graphs;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="32px" height="27px" viewBox="0 0 300 250" style="enable-background:new 0 0 300 250;" xml:space="preserve">
        <style type="text/css">
            .st0 {
                fill-rule: evenodd;
                clip-rule: evenodd;
