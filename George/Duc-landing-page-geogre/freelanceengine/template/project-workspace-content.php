...
<div class="workspace-project-box">
	<ul class="nav nav-tabs nav-tabs-workspace hidden-lg hidden-md" role="tablist">
		<li class="active"><a href="#workspace-conversation" data-group="conversation" data-toggle="tab" role="tab"><span><?php _e('Conversation', ET_DOMAIN); ?></span></a></li>
		<?php if (function_exists('ae_query_milestone') && $query->have_posts()) { ?>
			<li class="next"><a href="#workspace-milestone" data-group="files" data-toggle="tab" role="tab"><span><?php _e('Milestones', ET_DOMAIN); ?></span></a></li>
			<li><a href="#workspace-files" data-group="files" data-toggle="tab" role="tab"><span><?php _e('Project files', ET_DOMAIN); ?></span></a>
			</li>
		<?php } else { ?>
			<li class="next"><a href="#workspace-files" data-group="files" data-toggle="tab" role="tab"><span><?php _e('Project files', ET_DOMAIN); ?></span></a>
			</li>
		<?php } ?>
								...
								foreach ($comments as $key => $message) {
									...
									if (!$message->isFile) {
										if ($message->changed_milestone_id != '') {
											$partner_class =  ($message->user_id == $user_ID) ? '' : 'partner-message';
											echo '<li class="milestone-item-noti ' . $partner_class . '" id="comment-' . $message->comment_ID . '" data-commentTime="' . $message->comment_date_gmt . '">' . get_the_author_meta('display_name', $message->user_id) . ' ' . $convert->comment_content . '</li>';
										} else {
							?>
											<li class="<?php echo $message->user_id == $user_ID ? '' : 'partner-message' ?>" id="comment-<?php echo $message->comment_ID; ?>" data-commentTime="<?php echo $message->comment_date_gmt; ?>">
												...
												?>
													<div class="message-item message-item-file">
														<p>
															<a href="<?php echo wp_get_attachment_url($message->attachId); ?>" download>
																<?php
						...
						<?php if ($post->post_status == 'close' && ($user_ID == $post->post_author || $user_ID == $bid->post_author)) { ?>
							<form class="fre-workspace-form">
								<div class="conversation-typing">
									<textarea name="comment_content" class="content-chat" placeholder="<?php _e('Your message here...', ET_DOMAIN); ?>"></textarea>
								...
								<div class="conversation-submit-btn">
									<label class="conversation-send-file-btn" for="conversation-send-file">
										<div id="upload_file_container">
											...
											<span class="author_id" data-author="<?php echo $post->post_author ?>"></span>
											<a href="#" class="attack attach-file" id="upload_file_browse_button"><i class="fa fa-paperclip" aria-hidden="true"></i></a>
										</div>
									</label>
							...
							if ($lock_file != 'lock') { ?>
								<div class="workplace-title-arrow file-container" id="file-container" style="font-size: 0;">
									<div id="apply_docs_container">
										...
										<a href="#" class="workspace-add-files attack attach-file" id="apply_docs_browse_button"><i class="fa fa-plus"></i><span><?php _e('Add file', ET_DOMAIN); ?></span></a>
									</div>
								</div>
						<?php }
						...
						<ul class="workspace-files-list" id="workspace_files_list">
							...
							if (!empty($attachments)) {
								foreach ($attachments as $key => $value) {
									...
									<li class="freelancer-uploaded-item attachment-<?php echo $value->ID; ?>">
										<p><?php echo $value->post_title ?>
											<span>
												...
												if ($post->post_status == 'close' && $value->post_author == $user_ID && !$value->post_parent && (fre_share_role() || ae_user_role() == FREELANCER)) {
													echo '<a href="' . $value->guid . '" target="_blank" download><i class="fa fa-cloud-download" aria-hidden="true"></i></a>';
													if ($lock_file != 'lock') {
