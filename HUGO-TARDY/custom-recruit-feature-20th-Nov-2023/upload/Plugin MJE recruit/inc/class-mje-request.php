...
class MJE_Request extends AE_Post_Custom{
	...
	function __construct(){
        ...
        //custom seynou recruit code ( add expiration_date)
        $this->meta = array(
            'et_budget','time_delivery','expiration_date'
        );
    ...
    function convert($post, $thumbnail = 'medium_post_thumbnail', $excerpt = true, $singular = false){
        ...
        //custom seynou recruit code
        $recruit_category=wp_get_post_terms($post->ID,'mjob_category');
        foreach ( $recruit_category as $term ) {
            $recruit_category_name = $term->name;
        }
        $post->category_name=$recruit_category_name;
        //$post->recruit_expiration_date=show_human_time_expire_date(get_post_meta($post->ID,'expiration_date',true));   
        ...
        $post->recruit_avatar_author=$avatar;
        $recruit_author=get_userdata($post->post_author);
        $post->recruit_display_name=$recruit_author->display_name;
        $rate_score=mje_get_total_reviews_by_user($post->post_author);
        if($rate_score==0 || $rate_score=='')
        {
            $rate_text='<p class="text-notyet text-capitalize">not rated yet</p>';
        }
        else
        {
            $rate_text='<p class="text-rate-score text-capitalize text-center">'.number_format($rate_score, 1, '.', ' ').'</p>';  
        }
        $post->author_rate_text=$rate_text;
        $post->rating_score=$rate_score;
        $post->author_rate_score_number=number_format($rate_score, 1, '.', ' ');
        $post->time_of_delivery_txt='Delivery: '.$post->time_delivery.' days';
        //end
        return $post;
