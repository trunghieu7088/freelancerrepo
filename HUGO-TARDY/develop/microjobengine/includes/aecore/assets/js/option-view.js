...
(function(Models, Views, $, Backbone) {
        ...
        enable: function(e) {
            ...
            this.model.save('', '', {
                success: function() {
                    ...
                    AE.pubsub.trigger('ae:option:switch:change', {
    ...
    Views.Options = Backbone.View.extend({
        events: {
            ...
            'change textarea.regular-text': 'textChange',
             'change .dropdown'            : 'multiInputUpdate', // semantic ui  v1.3.9.9
             'click .js-copy-field': 'actCopyText', // v1.3.9.9
            ...
            'click a.reset-text-default': 'resetInput',
        ...
        textChange: function(e) {
            ...
            view.option.save('', '', {
                success: function(result, status, jqXHR) {
                    ...
                    if (status.success) {
                        $target.parent().append('<i class="fa fa-check" aria-hidden="true"></i>');
                        setTimeout(function() {
                            view.$('.fa-check').remove();
                        }, 2000);

                        AE.pubsub.trigger('ae:option:text:change', {
                            name: view.option.get('name'),
                            value: view.option.get('value')
                        });
                    } else {
        ...
        /** Add since V1.8.19 */
        multiInputUpdate: function(e) {
            var $target = $(e.currentTarget),
                form = $target.parents('form').find('select');
                var list = form.find("option:selected");
                var currencies = [];
                $.each( list, function( key, opt ) {
                    var currency = $(opt).val();
                    currencies.push(currency);
                });

            view = this;

            view.option.set('name', form.attr('name'));
            view.option.set('value',currencies );
        ...
        * add  V1.8.19
        */
        actCopyText :function(event){

            var $target = $(event.currentTarget);
            var $id = $target.attr('id');
            var $success = $target.attr('success');
        ...
        resetInput: function(e) {
            ...
            var $target = $(e.currentTarget),
                ...
                default_value = $target.attr('data-default'),
                option_name = $target.attr('data-name');
        ...
        toogleContent: function(event) {
            ...
            AE.pubsub.trigger('ae:option:field:toggle', $target, $form);
        },
        /**
         * change menu view
         */
    ...
    SettingRouter = Backbone.Router.extend({
        ...
        openSection: function(section) {
            ...
            if(content.length <= 0) {
                var targetId = $('.inner-menu').find('li:first-child a').attr('href');
                target = $('a[href="'+ targetId +'"]');
                content = $('#' + targetId.replace('#section/', ''));
            }
	...
	$.validator.addClassRules('positive_int_zero', {
        required: true,
        min: 0,
        number: true,
        digits: true
