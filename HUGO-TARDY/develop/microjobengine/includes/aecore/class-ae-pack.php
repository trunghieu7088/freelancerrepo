...
{
    static $instance;
    public $post_type, $localize, $convert, $option_name;
    ...
    public function et_package_cmp($a, $b)
    {
...
class AE_PackAction extends AE_Base
{
    ...
    public function pack_sync()
    {
        ...
        if (!isset($request['post_content'])) $request['post_content'] = __('content here', 'enginethemes');
        ...
        if ($result && !is_wp_error($result)) {
            ...
            wp_send_json(array(
                'success' => true,
                'data' => $result,
                'msg' => __("Sync success.", 'enginethemes')
            ));
        } else {
    ...
    function et_ajax_payment_sorting()
    {
        ...
        echo json_encode(array(
            'success' => $result,
            'msg' => __('Payment plans have been sorted', 'enginethemes')
        ));
        exit;
    }
}
