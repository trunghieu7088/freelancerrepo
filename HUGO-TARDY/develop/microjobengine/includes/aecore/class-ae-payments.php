...
abstract class AE_Payment extends AE_Base
{
    ...
    {
        et_track_payment('running: setup_payment');
        ...
        $msg_fail = __("Invalid payment gateway", 'enginethemes');
        if ($nvp['ACK']) {
            et_track_payment($nvp);
            $response = array(
                'success' => $nvp['ACK'],
                'data' => $nvp,
                'paymentType' => $paymentType
    ...
    {
        extract($data);
        if (!isset($payment_return['ACK']) || !$payment_return['ACK']) return false;
