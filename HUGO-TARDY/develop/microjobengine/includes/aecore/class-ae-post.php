...
class AE_Posts
{
     ...
     */
    public $current_post, $post_type, $taxs, $convert, $localize;
    ...
    public function convert($post_data, $thumbnail = 'medium_post_thumbnail', $excerpt = true, $singular = false)
    {
        ...
        if (!isset($post['ID']))
            return $post; // php8 added. 1.3.9.4
        ...
        $post_content = isset($result['post_content']) ? $result['post_content'] : '';
        $result['id'] = $post['ID'];
        $result['permalink'] = get_permalink($result['ID']);
        $result['unfiltered_content'] = $post_content;
        ...
        if (has_post_thumbnail($result['ID'])) {
            ...
            $result['the_post_thumnail'] = is_array($feature_image) ? $feature_image[0] : '';
        } else {
    ...
    function filter_content($content)
    {
        ...
        // $content = str_replace( '<a ', '<a rel="nofollow  noopener noreferrer" target="_blank"', $content ); 1.3.9.2
        $content = str_replace('<a ', '<a rel="nofollow  noopener noreferrer" ', $content); // 1.3.9.3
...
class AE_PostAction extends AE_Base
{
    ...
    function fetch_post()
    {
        ...
        if (is_array($data) && isset($data['query'])) {
            ob_start();
            $paginate = isset($_REQUEST['paginate']) ? $_REQUEST['paginate'] : '';
            ae_pagination($data['query'], $page, $paginate, $load_more_text);
