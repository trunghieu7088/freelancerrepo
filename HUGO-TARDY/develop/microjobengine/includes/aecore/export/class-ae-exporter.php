...
function mje_export( $args = array() ) {
	global $wpdb, $post;
	...
	$date        = gmdate( 'Y-m-d' );
	$wp_filename = $sitename . 'mjeOrder.' . $date . '.xml';
	/**
	...
	$sql = "SELECT p.ID FROM $wpdb->posts p  WHERE 1=1  AND p.post_type = 'mjob_order'  ORDER BY p.post_date DESC ";

	if( !empty($post_status) && !empty($payment_type)){

		$sql =  "SELECT p.ID FROM $wpdb->posts p
		INNER JOIN $wpdb->postmeta m ON ( p.ID = m.post_id )
		 WHERE 1=1  AND (
		  ( m.meta_key = 'payment_type' AND m.meta_value = '{$payment_type}' )
		) AND p.post_type = 'mjob_order' AND ((p.post_status = '{$post_status}'))
		 GROUP BY p.ID
		 ORDER BY p.post_date DESC";

	} else if( !empty($post_status) || !empty($payment_type)){
		if(!empty($post_status)){
			$sql = "SELECT p.ID  FROM wp_3_posts p  WHERE 1=1  AND
			p.post_type = 'mjob_order' AND ((p.post_status = 'publish'))  ORDER BY p.post_date DESC ";
		} else {
			$sql ="SELECT p.ID  FROM $wpdb->posts p
...
<rss version="2.0"
	xmlns:excerpt="http://wordpress.org/export/<?php echo ET_VERSION; ?>/excerpt/"
	xmlns:content="http://purl.org/rss/1.0/modules/content/"
	xmlns:wfw="http://wellformedweb.org/CommentAPI/"
	xmlns:dc="http://purl.org/dc/elements/1.1/"
	xmlns:wp="http://wordpress.org/export/<?php echo ET_VERSION; ?>/"
>
...
<channel>
	...
	<wp:wxr_version><?php echo ET_VERSION; ?></wp:wxr_version>
