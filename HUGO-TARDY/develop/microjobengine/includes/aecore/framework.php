<?php
class AppEngine extends AE_Base
{
    public $mail;
    public function __construct()
    {
        /**
         * add script appengine/core
         */
        $this->add_action('wp_enqueue_scripts', 'print_scripts', 9);
        $this->add_action('admin_enqueue_scripts', 'print_scripts');

        $this->add_action('wp_footer', 'override_template_setting', 101);

        if (isset($_REQUEST['page'])) {
            $this->add_action('admin_print_footer_scripts', 'override_template_setting', 200);
        }

        /**
         * Create a nicely formatted and more specific title element text for output
         * in head of document, based on current view.
         */
        $this->add_filter('wp_title', 'ae_wp_title', 10, 2);

        // When you are viewing on the Wordpress Administrator Panels will not use this filter
        if (!is_admin()) {
            /**
             * filter user avatar, replace by user upload avatar image
             */
            $this->add_filter('get_avatar', 'get_avatar', 10, 5);
        }
        /**
         * add ajax when user request thumbnail form view carousels
         */
        $this->add_ajax('ae_request_thumb', 'request_thumb');

        /**
         * add ajax when user request delete an image from gallery
         */
        $this->add_ajax('ae_remove_carousel', 'remove_carousel');

        /**
         * hook to action reject post and send mail to post author
         * @since 1.0
         * @author Dakachi
         */
        $this->add_action('ae_reject_post', 'reject_post');

        /**
         * hook to action ae insert post then send mail notify admin
         * @since 1.0
         * @author Dakachi
         */
        $this->add_action('ae_process_payment_action', 'notify_admin', 10, 2);


        add_theme_support('post-thumbnails');
        add_theme_support('automatic-feed-links');

        global $ae_post_factory;
        $ae_post_factory->set('post', new AE_Posts('post', array(
            'category'
        )));

        add_action('after_setup_theme', array(
            'AE_Language',
            'load_text_domain'
        ));

        $this->add_filter('nav_menu_css_class', 'special_nav_class', 10, 2);

        if (isset($_GET['close_notices'])) {
            update_option('option_sample_data', 1);
        }
    }

    /**
     * register base script
     */
    public function print_scripts()
    {

        $this->add_existed_script('jquery');

        $this->register_script('bootstrap', ae_get_url() . '/assets/js/bootstrap.min.js', array(
            'jquery'
        ), ET_VERSION, true);

        /**
         * bootstrap slider for search form
         */
        $this->register_script('slider-bt', ae_get_url() . '/assets/js/slider-bt.js', array(), true);

        $this->register_script('et-googlemap-api', '//maps.googleapis.com/maps/api/js?sensor=false&signed_in=false', '3.0', true);

        $this->register_script('ae-colorpicker', ae_get_url() . '/assets/js/colorpicker.js', array(
            'jquery'
        ));

        $this->register_script('gmap', ae_get_url() . '/assets/js/gmap.js', array(
            'jquery',
            'et-googlemap-api'
        ));

        $this->register_script('marker', ae_get_url() . '/assets/js/marker.js', array(
            'gmap'
        ), true);

        // tam thoi add de xai
        $this->register_script('jquery-validator', ae_get_url() . '/assets/js/jquery.validate.min.js', 'jquery');

        $this->register_script('marionette', ae_get_url() . '/assets/js/marionette.js', array(
            'jquery',
            'backbone',
            'underscore',
        ), true);

        // ae core js appengine
        if (!is_customize_preview()) {
            $this->register_script('appengine', ae_get_url() . '/assets/js/appengine.js', array(
                'jquery',
                'underscore',
                'backbone',
                'marionette',
                'plupload',
            ), ET_VERSION);
        }

        wp_localize_script('chosen', 'raty', array(
            'hint' => array(
                __('bad', 'enginethemes'),
                __('poor', 'enginethemes'),
                __('nice', 'enginethemes'),
                __('good', 'enginethemes'),
                __('gorgeous', 'enginethemes')
            )
        ));
        $adminurl = admin_url('admin-ajax.php');
        if (function_exists('icl_object_id') && defined('ICL_LANGUAGE_CODE')) {
            $current = ICL_LANGUAGE_CODE;
            $adminurl = admin_url('admin-ajax.php?lang=' . $current);
        }
        $max_upload_size = wp_max_upload_size();

        $variable = array(
            'ajaxURL' => $adminurl,
            'imgURL' => ae_get_url() . '/assets/img/',
            'assetImg' => get_template_directory_uri() . '/assets/img/',
            'jsURL' => ae_get_url() . '/assets/js/',
            'loadingImg' => '<img class="loading loading-wheel" src="' . ae_get_url() . '/assets/img/loading.gif" alt="' . __('Loading...', 'enginethemes') . '">',
            'loading' => __('Loading', 'enginethemes'),
            'ae_is_mobile'    => et_load_mobile() ? 1 : 0,
            'plupload_config' => array(
                'max_file_size' => ($max_upload_size / (1024 * 1024)) . 'mb',
                'url' => admin_url('admin-ajax.php'),
                'flash_swf_url' => includes_url('js/plupload/plupload.flash.swf'),
                'silverlight_xap_url' => includes_url('js/plupload/plupload.silverlight.xap'),
                'filters' => array(
                    array(
                        'title' => __('Image Files', 'enginethemes'),
                        'extensions' => 'jpg,jpeg,gif,png'
                    )
                )
            ),
            'homeURL' => home_url(),
            'is_submit_post' => is_page_template('page-post-place.php') ? true : false,
            'is_submit_project' => is_page_template('page-submit-project.php') ? true : false,
            'is_single' => (!is_singular('page') && is_singular()) ? true : false,
            'max_images' => ae_get_option('max_carousel', 5),
            'user_confirm' => ae_get_option('user_confirm') ? 1 : 0,
            'max_cat' => ae_get_option('max_cat', 3),
            'confirm_message' => __("Are you sure to archive this?", 'enginethemes'),
            'confirm_delete_message' => __("Are you sure to delete this?", 'enginethemes'),
            'map_zoom' => ae_get_option('map_zoom_default', 8),
            'map_center' => ae_get_option('map_center_default', array(
                'latitude' => 10,
                'longitude' => 106
            )),
            'fitbounds' => ae_get_option('fitbounds', ''),
            'limit_free_msg' => __("You have reached the maximum number of Free posts. Please select another plan.", 'enginethemes'),
            'error' => __("Please fill all require fields.", 'enginethemes'),
            'geolocation' => ae_get_option('geolocation', 0),
            'date_format' => get_option('date_format'),
            'time_format' => get_option('time_format'),
            'dates' => array(
                'days' => array(
                    __("Sunday", 'enginethemes'),
                    __("Monday", 'enginethemes'),
                    __("Tuesday", 'enginethemes'),
                    __("Wednesday", 'enginethemes'),
                    __("Thursday", 'enginethemes'),
                    __("Friday", 'enginethemes'),
                    __("Saturday", 'enginethemes'),
                    __("Sunday", 'enginethemes')
                ),
                'daysShort' => array(
                    __("Sun", 'enginethemes'),
                    __("Mon", 'enginethemes'),
                    __("Tue", 'enginethemes'),
                    __("Wed", 'enginethemes'),
                    __("Thu", 'enginethemes'),
                    __("Fri", 'enginethemes'),
                    __("Sat", 'enginethemes'),
                    __("Sun", 'enginethemes')
                ),
                'daysMin' => array(
                    __("Su", 'enginethemes'),
                    __("Mo", 'enginethemes'),
                    __("Tu", 'enginethemes'),
                    __("We", 'enginethemes'),
                    __("Th", 'enginethemes'),
                    __("Fr", 'enginethemes'),
                    __("Sa", 'enginethemes'),
                    __("Su", 'enginethemes')
                ),
                'months' => array(
                    __("January", 'enginethemes'),
                    __("February", 'enginethemes'),
                    __("March", 'enginethemes'),
                    __("April", 'enginethemes'),
                    __("May", 'enginethemes'),
                    __("June", 'enginethemes'),
                    __("July", 'enginethemes'),
                    __("August", 'enginethemes'),
                    __("September", 'enginethemes'),
                    __("October", 'enginethemes'),
                    __("November", 'enginethemes'),
                    __("December", 'enginethemes')
                ),
                'monthsShort' => array(
                    __("Jan", 'enginethemes'),
                    __("Feb", 'enginethemes'),
                    __("Mar", 'enginethemes'),
                    __("Apr", 'enginethemes'),
                    __("May", 'enginethemes'),
                    __("Jun", 'enginethemes'),
                    __("Jul", 'enginethemes'),
                    __("Aug", 'enginethemes'),
                    __("Sep", 'enginethemes'),
                    __("Oct", 'enginethemes'),
                    __("Nov", 'enginethemes'),
                    __("Dec", 'enginethemes')
                )
            )
        );
        $variable['global_map_style'] = AE_Mapstyle::get_instance()->get_current_style();
        $variable = apply_filters('ae_globals', $variable);
        wp_localize_script('appengine', 'ae_globals', $variable);

        /**
         * html5
         */


        // Loads the Internet Explorer specific stylesheet.
        if (!is_admin()) {
            echo '<!--[if lt IE 9]>
                <script src="' . ae_get_url() . '/assets/js/html5.js"></script>
            <![endif]-->';
            $this->register_style('bootstrap', ae_get_url() . '/assets/css/bootstrap.min.css', array(), '3.0');
        }


        /**
         * Current user
         */
        global $current_user;
        $current_user = wp_get_current_user();
        if (isset($current_user->ID) && $current_user->ID != 0) {
            $user_obj = new AE_Users();
            $user_data  = $user_obj->convert($current_user->data);
            $current_user->data = $user_data;
        }
?>
        <script type="text/javascript" id="current_user">
            var currentUser = <?php if (isset($current_user->ID) && $current_user->ID != 0) echo wp_json_encode($current_user);
                                else echo wp_json_encode(array(
                                    'id' => 0,
                                    'ID' => 0
                                )); ?>;
            var is_mobile = <?php echo wp_json_encode(et_load_mobile()); ?>;
        </script>
    <?php
    }

    /**
     * add script to footer override underscore templateSettings, localize validator message
     */
    function override_validator_errors()
    {
    ?>
        <!-- localize validator -->
        <script type="text/javascript">
            (function($) {
                if (typeof $.validator !== 'undefined') {
                    $.extend($.validator.messages, {
                        required: "<?php _e("This field is required.", 'enginethemes') ?>",
                        email: "<?php _e("Please enter a valid email address.", 'enginethemes') ?>",
                        url: "<?php _e("Please enter a valid URL.", 'enginethemes') ?>",
                        number: "<?php _e("Please enter a valid number.", 'enginethemes') ?>",
                        digits: "<?php _e("Please enter only digits.", 'enginethemes') ?>",
                        equalTo: "<?php _e("Please enter the same value again.", 'enginethemes') ?>",
                        date: "<?php _e("Please enter a valid date.", 'enginethemes'); ?>",
                        creditcard: "<?php _e("Please enter a valid credit card number.", 'enginethemes'); ?>",
                        accept: "<?php _e("Please enter a value with a valid extension.", 'enginethemes'); ?>",

                        maxlength: $.validator.format("<?php _e("Please enter no more than {0} characters.", 'enginethemes'); ?>"),
                        minlength: $.validator.format("<?php _e("Please enter at least {0} characters.", 'enginethemes'); ?>"),
                        rangelength: $.validator.format("<?php _e("Please enter a value between {0} and {1} characters long.", 'enginethemes'); ?>"),
                        range: jQuery.validator.format("<?php _e("Please enter a value between {0} and {1}.", 'enginethemes'); ?>"),
                        min: $.validator.format("<?php _e("Please enter a value greater than or equal to {0}.", 'enginethemes'); ?>"),
                        max: $.validator.format("<?php _e("Please enter a value less than or equal to {0}.", 'enginethemes'); ?>")
                    });
                }
            })(jQuery);
        </script>
        <?php
    }

    function override_template_setting()
    {
        $this->override_validator_errors();

        // print google analytics code
        if (!is_admin()) {
            echo stripslashes(ae_get_option('google_analytics'));
            // user confirm scripts
            if (isset($_GET['act']) && $_GET['act'] == "confirm" && $_GET['key']) {
                $user_id = AE_Users::confirm($_GET['key']);
                if ($user_id) {
                    $mail = AE_Mailing::get_instance();
                    $mail->confirmed_mail($user_id);

                    $time = 0;
                    $time = apply_filters('ae_confirm_user_time_out', $time);

        ?>
                    <script type="text/javascript" id="user-confirm">
                        (function($, Views, Models, AE) {
                            $(document).ready(function() {
                                AE.pubsub.trigger('ae:notification', {
                                    msg: "<?php _e("Your account has been confirmed successfully!", 'enginethemes')  ?>",
                                    notice_type: 'success',
                                });

                                setTimeout(function() {
                                    window.location.href = "<?php echo home_url(); ?>";
                                }, <?php echo $time ?>);
                            });
                        })(jQuery, AE.Views, AE.Models, window.AE);
                    </script>
<?php }
            }
        }
    }

    /**
     * Create a nicely formatted and more specific title element text for output
     * in head of document, based on current view.
     *
     * @since AE 1.0
     *
     * @param string $title Default title text for current view.
     * @param string $sep Optional separator.
     * @return string The filtered title.
     */
    function ae_wp_title($title, $sep)
    {
        global $paged, $page;

        if (is_feed()) {
            return $title;
        }

        // Add the site name.
        $title .= get_bloginfo('name');

        // Add the site description for the home/front page.
        $site_description = get_bloginfo('description', 'display');
        if ($site_description && (is_home() || is_front_page())) {
            $title = "$title $sep $site_description";
        }

        // Add a page number if necessary.
        if ($paged >= 2 || $page >= 2) {
            $title = "$title $sep " . sprintf(__('Page %s', 'enginethemes'), max($paged, $page));
        }

        return $title;
    }

    /**
     * filter wp avatar use AE_Users return a image tag with user setting avatar url
     * @param $avatar
     * @param $id_or_email
     * @param $size
     * @author Dakachi
     * @version 1.0
     */
    function get_avatar($avatar, $id_or_email, $size, $default, $alt)
    {
        $seller = AE_Users::get_instance();

        $profile_picture = $seller->get_avatar($id_or_email, $size, $default);

        /**
         * overide $default by profile picture
         */
        if ($profile_picture != '') {
            $default = $profile_picture;
            if (false === $alt) $safe_alt = '';
            else $safe_alt = esc_attr($alt);

            $avatar = "<img alt='{$safe_alt}' src='{$default}' class='avatar avatar-{$size} photo avatar-default' height='{$size}' width='{$size}' />";
        }
        return $avatar;
    }

    /**
     * request carousel thumbnail image for edit ad form
     * send json back carousel js view
     * @author Dakachi
     * @version 1.0
     */
    function request_thumb()
    {
        $items = isset($_REQUEST['item']) ? $_REQUEST['item'] : array();
        $return = array();
        $size = apply_filters('ae_request_thumbnail_size', array(
            'thumbnail'
        ));
        if (!empty($items)) {
            foreach ($items as $key => $value) {
                $return[] = et_get_attachment_data($value, $size);
            }
            wp_send_json(array(
                'success' => true,
                'data' => $return
            ));
        } else {
            wp_send_json(array(
                'success' => false
            ));
        }
    }

    /**
     * request remove image for edit ad form
     * send json back carousel js view
     * @author Dakachi
     * @version 1.0
     */
    function remove_carousel()
    {
        if (!current_user_can('manage_options')) {
            global $user_ID;
            $post = get_post($_REQUEST['id']);
            if ($user_ID != $post->post_author) wp_send_json(array(
                'success' => false,
                'msg' => __("Not owned this image!", 'enginethemes')
            ));
        }
        wp_delete_post($_REQUEST['id'], true);
        $meta = get_post_meta($_REQUEST['id']);
        if (!empty($meta)) {
            foreach ($meta as $meta_key => $meta_value) {
                delete_post_meta($_REQUEST['id'], $meta_key, null);
            }
        }
        wp_send_json(array(
            'success' => true
        ));
    }

    /**
     * reject post
     * @param $data
     */
    function reject_post($data)
    {
        $this->mail = AE_Mailing::get_instance();
        $this->mail->reject_post($data);
    }
    /**
     * send notify to admin
     * @param Object $post Post data
     * @since 1.1
     * @author Dakachi
     */
    function notify_admin($payment_return, $data)
    {
        if (!isset($data['ad_id'])) return false;
        if (!$payment_return['ACK']) return;
        $this->mail = AE_Mailing::get_instance();
        $this->mail->new_post_alert($data['ad_id']);
    }

    function special_nav_class($classes, $item)
    {
        if (in_array('current-menu-item', $classes)) {
            $classes[] = 'active ';
        }
        return $classes;
    }
}
global $et_appengine;
$et_appengine = new AppEngine();
