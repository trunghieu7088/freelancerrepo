...
function ae_currency_sign($echo = true)
{
    ...
    if ($echo)
        ...
        return $icon; // return an option value
}
...
function et_the_time($from)
{
    ...
    //
    if (current_time('timestamp')  - $from > (7 * 24 * 60 * 60)) {
...
{
    $page     = ae_get_option('social_connect', false);
    if (!$page || empty($page)) {
        $page_args = array(
        ...
        $page = array($page_id);
        ae_update_option('social_connect', array($page));
    } else {
        $page_id = $page[0];
    }
    $link = get_permalink($page_id);

    return apply_filters('ae_get_social_connect_page_link', get_permalink($page_id));
}
/**
