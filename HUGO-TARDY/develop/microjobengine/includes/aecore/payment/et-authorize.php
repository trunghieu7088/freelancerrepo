...
class ET_Authorize extends ET_Payment {
	...
	function __construct( $settings = array (), $mode = NULL ){
	...
	static function set_api ( $api = array () ) {
		...
		if(!self::is_enable()) {
			...
			if(isset($gateways['authorize']['active']) && $gateways['authorize']['active'] != -1 ) {
				...
				return __('Your Authorize.Net was disabled because of invalid settings!', 'enginethemes');
			}
		}
		return true;
	}
...
function et_add_authorize_gateway ( $support) {
	$support['authorize']		= array ('label' => 'Authorize.Net', 'active' => -1,
									'description' => __("Send your payment via Authorize.Net",'enginethemes'));
	return $support;
}
