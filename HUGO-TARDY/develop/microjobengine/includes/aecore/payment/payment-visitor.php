...
abstract class  ET_PaymentVisitor
{
	...
	{
		$this->_order 	= (is_array($order) || is_object($order)) ? $order : new stdClass();
	}
...
class ET_PaypalVisitor extends ET_PaymentVisitor
{
	...
	function setup_checkout(ET_Order $order)
	{
		et_track_payment('ET_PaypalVisitor::setup_checkout');
	...
	function do_checkout(ET_Order $order)
	{
		ae_paypal_log('processing ET_PaypalVisitor::do_checkout() ');
	...
	{
		ae_paypal_log('Processing ::do_checkout_get_back()');
		ae_paypal_log("IPN post back: ");
		ae_paypal_log($_POST);
		 ...
		 * check PDT
		 */
		ae_paypal_log("Request data in checkout get back: ");
		ae_paypal_log($_REQUEST);
		if (isset($_REQUEST['st']) && ($_REQUEST['st'] == 'Completed' || $_REQUEST['st'] == 'Pending')) {
			...
			if ($_REQUEST['amt'] == $order['total'] && $_REQUEST['cc'] == $order['currencyCodeType']) {
				...
				ae_paypal_log('update order to publish here.');
				/**
		...
		if (isset($_POST['payment_status'])  && ($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Pending')) {
			...
			if (
				$mc_gross == $order['total'] && $mc_currency == $order['currencyCodeType']
				&&  ($receiver_email == trim($api['api_username']) || $business == trim($api['api_username']))
			) {
				ae_paypal_log('verify paypal SUCCESS.');
				...
				ae_paypal_log('verify paypal FAIL.');
				return array(
					'ACK'		=> false,
					'payment'	=>	'simplePaypal',
	...
	function setup_checkout(ET_Order $order)
	{
		...
		if (!empty($products)) {
			foreach ($products as $key => $value) {
				...
				$pro_str	.=	"&c_name_$k=" . urldecode($value['NAME']);
				$pro_str	.=	"&c_description_$k=" . urldecode($value['NAME']);
