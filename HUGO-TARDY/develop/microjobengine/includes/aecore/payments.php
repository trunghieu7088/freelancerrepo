...
if (!function_exists('ae_member_process_order')) {
     ...
     * @author Dakachi
     */
    function ae_member_process_order($user_id, $order, $data)
    {
        $mail = AE_Mailing::get_instance();
        $mail->send_receipt($user_id, $order, $data);
    }
}
add_action('ae_member_process_order', 'ae_member_process_order', 10, 3);
...
?>
                <p>
                    ...
                    if ($order->post_status == 'publish') printf(__("You purchased package <strong>%s</strong> and have %d post/s left.", 'enginethemes'), $package->post_title, $number_of_post);
                    if ($order->post_status == 'pending') printf(__("You purchased package <strong>%s</strong> and have %d post/s left. Your posted post is pending until payment.", 'enginethemes'), $package->post_title, $number_of_post);
                    ?>
                </p>
