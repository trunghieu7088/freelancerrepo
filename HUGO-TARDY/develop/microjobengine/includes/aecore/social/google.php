...
class ET_GoogleAuth extends ET_SocialAuth
{
    ...
    public function __construct() {
        parent::__construct('google', 'et_google_id', array(
            ...
            'content' => __("This seems to be your first time signing in using your Google+ account.If you already have an account, please log in using the form below to link it to your Google+ account. Otherwise, please enter an email address and a password on the form, and a username on the next page to create an account.You will only do this step ONCE. Next time, you'll get logged in right away.", 'enginethemes') ,
        ...
        if( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'gplus_auth_callback' ){
            if (!empty($this->gplus_client_id ) && !empty($this->gplus_secret_key) /*&& !is_user_logged_in()*/){
                ...
                _e( 'Please enter your Google client id and secret key in setting page!', 'enginethemes' );
                exit();
            }
        }
    }
        ...
        if( ( isset( $_REQUEST['code'] ) && !empty( $_REQUEST['code'] ) ) && ( isset( $_REQUEST['state'] ) || $_REQUEST['state'] == $this->state ) ){
            try {
                ...
                if( isset( $remote_post ['body'] ) ){
                    ...
                    _e( 'Error to connect to Google Server!', 'enginethemes' );
                    exit();
                }
                /**
                ...
                if( isset( $data->access_token ) ){
                    ...
                    _e( 'Error to connect to Google', 'enginethemes' );
                    exit();
                }
