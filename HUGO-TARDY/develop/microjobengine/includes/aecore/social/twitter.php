...
class ET_TwitterAuth extends ET_SocialAuth
{
    ...
    public function __construct() {
        parent::__construct('twitter', 'et_twitter_id', array('title' => __("SIGN IN WITH TWITTER", 'enginethemes'), 'content' => __("This seems to be your first time signing in using your Twitter account.If you already have an account  , please log in using the form below to link it to your Twitter account. Otherwise, please enter an email address and a password on the form, and a username on the next page to create an account.You will only do this step ONCE. Next time, you'll get logged in right away.", 'enginethemes'), 'content_confirm' => __("Please provide a username to continue", 'enginethemes')));
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'twitterauth') {
            ...
            if ($request_token) {
                ...
                switch ($connection->http_code) {
                    ...
                    default:
                        _e("Conection with twitter Failed", 'enginethemes');
                        exit;
                        break;
                }
            }
            else {
                echo __("Error Receiving Request Token", 'enginethemes');
                exit;
            }
        }
        ...
        // create user
            ...
            if (!empty($_POST['user_email'])) {
                ...
                if (!is_wp_error($result)) {
                    ...
                    //  __("You have been logged in via Twitter", 'enginethemes'),
