...
class WC_Integrate
{
    ...
    public function add_hook()
    {
        ...
        //add_action('mje_after_payment_list', array(self::getInstance(), 'add_wc_payment')); v1392 disabled
    }
        ...
        foreach ($available_payment_gateways as $key => $gateway):
            ...
            <li>
                ...
                <a href="#" class="btn btn-submit-price-plan select-payment"
                   data-type="<?php echo $key ?>"><?php _e("Select", 'enginethemes'); ?></a>
            </li>
        <?php
        endforeach;
    }
 ...
 */
 // add_action('init', 'ae_payment_postback_handle', 0);
 function ae_payment_postback_handle() {
     ...
     if ( isset( $_REQUEST['txn_type']) && !empty( $_REQUEST['txn_type'] )  ) {

        et_track_payment('ae_payment_postback_handle');
