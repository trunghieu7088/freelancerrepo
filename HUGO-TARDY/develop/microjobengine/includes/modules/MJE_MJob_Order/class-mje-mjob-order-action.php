...
class MJE_MJob_Order_Action extends MJE_Post_Action
{
    ...
    public function sync($request)
    {
        ...
        if ($response['success']) {
            ...
            if ($request['method'] == 'create') {
                ...
                update_post_meta($result->ID, 'fee_commission', (float) ae_get_option('order_commission_buyer', 0)); // %
                update_post_meta($result->ID, 'buyer_id', $result->post_author);

                //update seller commission
                update_post_meta($result->ID, 'seller_commission', (float) ae_get_option('order_commission', 0)); // %

                //calculate fixed value of seller commission and update
                $seller_commission_value = $subtotal - mje_get_price_after_commission($subtotal);
                //$seller_commission_value= 100 * 11.16 / 93 - 11.16;                 
                update_post_meta($result->ID, 'seller_commission_value', $seller_commission_value);
                update_post_meta($result->ID, 'seller_gross_income', $subtotal);
                ...
                if (isset($request['custom_offer_id'])) {
                    ...
                    //calculate fixed value of buyer commission if this is custom order
                    $custom_offer_price = get_post_meta($request['custom_offer_id'], 'custom_offer_budget', true);
                    $fee_commission_value = $custom_offer_price * ae_get_option('order_commission_buyer', 0) / 100;
                } else {
                    //calculate fixed value of buyer commission if this is normal order
                    $fee_commission_value = $mjob->et_budget * ae_get_option('order_commission_buyer', 0) / 100;
                }

                //calculate with extra services price if they exists ( buyer commission )
                if (!empty($result->extra_ids)) {
                    foreach ($result->extra_ids as $key => $value) {
                        $fee_commission_value += get_post_meta($value, 'et_budget', true) * ae_get_option('order_commission_buyer', 0) / 100;
                    }
                }

                //update fixed value of buyer commission
    ...
    public function get_seller_cross_revenue($mjob_order)
    {
