...
class mJobOrderSchedule extends AE_Base{
    ...
    public function cronAction(){
        ...
        $sql = "SELECT * FROM {$wpdb->posts} AS p
				...
				WHERE 	(p.post_type = '{$post_type}') 	AND
						...
						( (TIMESTAMPDIFF(MINUTE, mt.meta_value, '{$current}' ) ) >= {$duration} * 1440 ) AND
						(mt.meta_value != '' )";
        ...
        foreach($posts as $post) {
            ...
            $mail->auto_finish_order($post);//@param array order

            // Create change log
            mje_add_mjob_order_changelog($order->ID, $order->post_author, 'auto_finish');
        }
    }
    /**
     * add cron time
    ...
    public function add_cron_time( $schedule ){
        $schedule[$this->cron_name] = array(
            ...
            'display'=> apply_filters('mjob_cron_display', __('Finish microjob order in duration', 'enginethemes'), $this->cron_name)
        );
        return $schedule;
    }
    /**
