...
function form_request_personal_data( $atts, $content = null ) {
	if( isset( $_POST['submit'] ) ){
		...
		if( $request_id &&  !is_wp_error($request_id) ){
			...
			_e('Your request is sent.','enginethemes');
		} else {
		...
		<form method="post" class="wp-privacy-request-form ae-gdpr-form-js">
			<h2><?php _e( 'Submit Data Export Request','enginethemes' ); ?></h2>
			<p><?php _e( 'An email will be sent to the user at this email address to verify the request.','enginethemes' ); ?></p>
			...
			<div class="wp-privacy-request-form-field" class="form-group " ><br />
				<button type="submit" name="submit" id="submit" class="fre-btn  btn-save  waves-effect waves-light btn-submit primary-bg-color" value="1"><?php _e('Send Request','enginethemes');?></button>
			</div>
