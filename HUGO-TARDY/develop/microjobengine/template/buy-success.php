...
<div id="content" class="mjob-order-page mjob-single-page">
    <div class="block-page">
        <div class="container float-center">
            <p class="block-title no-margin"><?php _e('PAYMENT SUCCESS', 'enginethemes'); ?></p>
                ...
                if($user_ID):
                    if($order_id):
                        ...
                        if($payment_type == 'cash'){
                            printf(__("<p>Your listing has been submitted to our website.</p> %s ", 'enginethemes') , $response['L_MESSAAGE']);
                        } ?>
                    <p><?php _e("You are now redirected to your order detail page",'enginethemes');?></p>
                       <p class="time-leave"><?php printf(__('Time left: %s', 'enginethemes' ), '<span class="count_down">10</span>');  ?></p>
                    <?php echo '<a href="'.$permalink.'" class="btn-submit btn-view-order" >'.__("View your order", 'enginethemes').'</a>';
                    else:
                        _e('You buy a service success!', 'enginethemes');
                    endif;
                    et_destroy_session();
                else:
                    $signin_text = __('Please login to view your order detail', 'enginethemes');
                    $signup_text = __('Register an account to handle your order', 'enginethemes');
