...
<div id="comments" class="project-comments comments-area et-comments-area">
	...
	<h3 class="title et-comments-title">
		<?php
			if($total_count == 0){
				_e("0 Comments", 'enginethemes');
			}else if($total_count == 1){
				printf(__("%d Comment", 'enginethemes'), intval($total_count));
			}else {
				printf(__('%d Comments', 'enginethemes'), $total_count);
			}
		?>
	</h3>
	...
	<nav id="comment-nav-below" class="navigation comment-navigation" role="navigation">
		<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'enginethemes' ) ); ?></div>
		<div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'enginethemes' ) ); ?></div>
	...
	<p class="no-comments et-comments-title">
		<?php _e( 'Comments are closed.', 'enginethemes' ); ?>
	</p>
	<?php endif; ?>
	<div class="btm-comment-form">
		<?php
			comment_form ( array(
							'comment_field'        => ' <div class="form-item"><label for="comment">' . __( 'Your Comment', 'enginethemes' ) . '</label><div class="input"><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></div></div>',
							'comment_notes_before' => '',
							'comment_notes_after'  => '',
							'id_form'              => 'commentform',
							'id_submit'            => 'submit',
							'title_reply'          => __( "Comment", 'enginethemes'),
							'title_reply_to'       => __( 'Leave a Reply to %s', 'enginethemes'),
							'cancel_reply_link'    => __( 'Cancel reply','enginethemes' ),
							'label_submit'         => __( 'Comment', 'enginethemes' ),
        ...
        <div id="comment-<?php comment_ID(); ?>">
            ...
            <div class="media-body">
                ...
                <span class="fre-comment-label">
                	<?php _e('The comment is awaiting moderation.', 'enginethemes'); ?>
                </span>
                <?php } ?>
                </h4>
                ...
                <?php
                    comment_reply_link(array_merge($args, array(
						'reply_text' => __( 'Reply ', 'enginethemes' ),
						'depth'      => $depth,
