...
?>
	...
	public function checkout_claim($request)
	{
		...
		//$claim->amount = $total;
		$ae_option = new AE_Options;
		$claim->amount =($ae_option->get_option('claim_price_fixed_value') <> "") ? $ae_option->get_option('claim_price_fixed_value') : 1;
		//end custom code
		$resp['data'] = $claim;
		return $resp;
	}
					...
					<# } #>
						...
						public function mje_seller_mjob_button_function()
						{
							...
							if ($mje_claim->can_claim($post->ID)) {
								...
								$fee = ($ae_option->get_option('claim_fee') <> "") ? $ae_option->get_option('claim_fee') : 10;
								//custom code 18th jun 2024 ( set fixed value from backend for price claim)
								//$price_claim = round($price * ($fee / 100), 2);
								$price_claim = ($ae_option->get_option('claim_price_fixed_value') <> "") ? $ae_option->get_option('claim_price_fixed_value') : 1;
								//end custom code
							?>
