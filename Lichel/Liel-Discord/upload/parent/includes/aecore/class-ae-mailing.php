...
class AE_Mailing extends AE_Base
{
    ...
    public function wp_mail($to, $subject, $content, $filter = array() , $headers = '') {
        ...
        //custom code for discord
        $user_get_noti=get_user_by('email',$to);
        $UserDiscordID=get_user_meta($user_get_noti->ID,'DiscordID',true);
        $et_discord_subscribe =  get_user_meta($user_get_noti->ID,'et_discord_subscribe', true);
        if($UserDiscordID && ($et_discord_subscribe=='1' || $et_discord_subscribe===1 || $et_discord_subscribe==1))
        {
        	$discordContent=strip_tags($content);
        	do_action('sendMessageDiscord',$UserDiscordID,$discordContent);        
        }
