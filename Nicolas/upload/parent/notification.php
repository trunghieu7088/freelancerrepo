...
class Fre_Notification extends AE_PostAction
{
	...
	function fre_notify_item($notify)
	{
		...
		switch ($type) {
			//custom code here 
			case 'discount_pack_noti':

			$fre_pack_code_link=site_url('/my-project/');
				
				$message        = sprintf( __( 'Admin has sent you a free pack : ', ET_DOMAIN ));
				$discount_pack_sku=get_post_meta($notify->ID,'discount_pack_sku',true);			
				$pack_name = apply_filters('getPackNoti',$discount_pack_sku);
				
					if ( ! isset( $sender ) ) {
					$sender = 1;
				}
				$content.= '<a class="fre-notify-wrap" href="' . $fre_pack_code_link . '">
                                 <span class="notify-avatar">' .'<span class="noti-icon"><i class="fa fa-info-circle"></i></span>'. '</span>
                                <span class="notify-info">' . 	$message.'<strong>'.$pack_name.'</strong>'.'</span>
				...
				break;	
									...
									case 'discount_code_noti':

			$fre_discount_code_link=site_url('/discount-list/');
				
				$message        = sprintf( __( 'Admin has sent you a discount code ', ET_DOMAIN ));
				$discount_code=get_post_meta($notify->ID,'discount_code_noti',true);
				$discount_percent=get_post_meta($notify->ID,'discount_percent_noti',true);
				$discount_percent_text=$discount_percent.'%';
					if ( ! isset( $sender ) ) {
					$sender = 1;
				}
				$content        .= '<a class="fre-notify-wrap" href="' . $fre_discount_code_link . '">
                                 <span class="notify-avatar">' .'<span class="noti-icon"><i class="fa fa-info-circle"></i></span>'. '</span>
                                <span class="notify-info">' . 	$message.$discount_percent_text.' '. $discount_code . '</span>
                                <span class="notify-time">' . sprintf( __( "%s on %s", ET_DOMAIN ), get_the_time( '', $notify->ID ), get_the_date( '', $notify->ID ) ) . '</span>
                            </a>';
				break;	
