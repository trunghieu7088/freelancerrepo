    ...
    **/
	...
	function enqueue_script(){
		if( is_page_template('page-profile.php') || $this->is_membership_checkout_page() ){
			...
			wp_enqueue_script( 'customdiscount', MEMBERSHIP_URL. '/assets/custom_discount.js', array(
				'jquery',
				'underscore',
				'backbone',
				'appengine'
	...
	function show_subscription_info($user_role){
		...
		if( $subscription ){
			...
			if( $subscription->auto_renew == 'active' ){
				...
				//custom discount here
				$btn_html.=' <a href="javascript:void(0)" class="fre-normal-btn-o openDiscountModal">'.__('Apply Discount','enginethemes').'</a>';
				//end
			}
