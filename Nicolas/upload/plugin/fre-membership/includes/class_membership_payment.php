...
Class Fre_Membership_Payment{
	...
	function add_credit_checkout_button($pack){
		...
		if( $this->enable_credit ){
			...
			<?php } else{ ?>
				<li class="panel credit-payment-gateway">
					<span class="title-plan" data-type="credit"><?php _e('Credit','enginethemes');?><span><?php printf(__('System auto deduct %s in your credit balance.','enginethemes'), '<strong id="custom_price2">'.fre_price_format($price).'</strong>'  );?></span></span>
	...
	function subscriberViaCredit(){
		...
		//custom code here
		if(isset($_POST['discount_code_id_credit']) && $_POST['discount_code_id_credit'] != '')
		{
			$discount_code_info=get_post($_POST['discount_code_id_credit']);
			if(!empty($discount_code_info))
			{
				$discount_percent=get_post_meta($_POST['discount_code_id_credit'],'discount_percent',true);
				$decreased_price=($discount_percent*$pack->et_price)/100;
				$discounted_price=$pack->et_price-$decreased_price;
				$pack->et_price=$discounted_price;
			}
        ...
        update_post_meta($_POST['discount_code_id_credit'],'used','used');
        // saveHistory
	...
	function add_modal_on_footer(){
		...
		if( is_page_template('page-profile.php') ){
			...
			fre_memhership_template_part('templates/modal','renew-discount');
			//end
		}
