...
class Fre_Notification extends AE_PostAction
{
	...
	function fre_notify_item($notify)
	{
		...
		$no_project_types = apply_filters('none_project_types', array(
			...
			'minus_credit',
			'membership',
			//custom code add freelancer by admin here
			'admin_add_freelancer_pj'
			//end
		));
		...
		switch ($type) {
			...
			//custom code for add freelancer here
			case 'admin_add_freelancer_pj':
			$project_guid=get_post_meta($notify->ID,'project_guid',true);
			$project_bid=get_post($notify->post_parent);
			$content.= '<a class="fre-notify-wrap" href="' . $project_guid.'">';
			$content.= '<span class="notify-avatar">' .'<span class="noti-icon"><i class="fa fa-info-circle"></i></span>'. '</span>';
			$content.= '<span class="notify-info">'.$notify->post_title.' '.'<strong>'. $project_bid->post_title .'</strong></span>';
			$content.= ' <span class="notify-time">' . sprintf( __( "%s on %s", ET_DOMAIN ), get_the_time( '', $notify->ID ), get_the_date( '', $notify->ID ) ) . '</span>';
			$content.='</a>';
			break;	
			//end
