...
class Fre_ProjectAction extends AE_PostAction
{
		...
		if ($method == 'archive') {
			...
			if (isset($project_id) && $project_id != '') {
				...
				if (!($result)) {
					wp_send_json(array(
						'success' => false,
						'msg'     => __("Failed to delete project!", ET_DOMAIN)
					));
				} else {
					//update bid status
					$bids_post = get_children(
		...
		if ($project_id) {
			if ($type == 'lock') {
				...
				if (!($result)) {
					wp_send_json(array(
						'success' => false,
						'msg'     => __("Failed to lock project files!", ET_DOMAIN)
					));
				} else {
				...
				if (!($result)) {
					wp_send_json(array(
						'success' => false,
						'msg'     => __("Failed to unlock project files!", ET_DOMAIN)
					));
				} else {
	...
	function ae_convert_project($result)
	{
		...
		//custom code project list
		$custom_post = get_post($result->ID);		 
		$human_readable_time = human_time_diff(strtotime($custom_post->post_date), current_time('timestamp')).' ago';
		$result->human_readable_time=$human_readable_time;
		$result->custom_avatar_project       = get_avatar($result->post_author, 80);
		$result->project_posted = fre_count_user_posts_by_type($result->post_author, 'project', '"publish","complete","close","disputing","disputed" ', true);
		//end custom code
 ...
 * @return mixed sum of bid budget on this project.
 */
		...
		<li>
			<div class="address">
				<span class="addr-wrap">
					...
					<span class="info addr" title="<?php echo isset($user_data->location) ? $user_data->location : ''; ?>">
		...
		<ul class="list-info-company-details">
			<?php if ($role == 'freelancer') { ?>
				<li>
					<div class="address">
						<span class="addr-wrap">
							<span class="title-name"><i class="fa fa-user"></i><?php _e('Address:', ET_DOMAIN); ?></span>
							<span class="info addr" title="<?php echo isset($user_data->location) ? $user_data->location : ''; ?>">
								<?php echo isset($user_data->location) ? $user_data->location : ''; ?>
							</span>
						</span>
				...
				<li>
					<div class="member-since">
						<span class="member-wrap">
							<span class="title-name"><i class="fa fa-user"></i><?php _e('Member Since:', ET_DOMAIN); ?></span>
