        ...
        submitPost: function(event) {
            ...
            if (!view.model.get('uploadingCarousel')) {
                if( $("form.post").valid() && view.customValidate()) {
                    //custom code validate short video code 31 oct 2024
                    //if the users choose video upload type but did not upload video --> not allow submit
                    let short_video_ids=$("#short_video_attach_id").val();
                    if($("#choosenvideoType").val()=='upload' && (!short_video_ids || short_video_ids==''))
                    {
                        AE.pubsub.trigger('ae:notification', {
                            msg: 'Please choose video to upload',
                            notice_type: 'error',
                       });
                       
                        $('html, body').animate({
                                    scrollTop: $("#video_caption").offset().top
                        }, 500);
                        return ;
                    }
                    //end validate short video
                    //end custom code short video
                    view.model.save('', '', {
                        beforeSend: function () {
