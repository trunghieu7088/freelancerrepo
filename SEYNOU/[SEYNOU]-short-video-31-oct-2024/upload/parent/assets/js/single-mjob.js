    ...
    $(function() {
        ...
        Views.EditMjob = Backbone.View.extend({
                        ...
                        if (res.success) {
                            ...
                            $('.mjob-edit-content').find('.loading').hide();
                             ...
                             //end custom code
                              ...
                              //custom code short video 31th oct 2024
                             let video_player_html= res.data.video_html_render;
                             if(video_player_html)                    
                             {
                                $("#mjob-edit-video-area").append(video_player_html);
                                const mjobEditVideo = new Plyr("#mjob-edit-video-player",{ 
                                    controls: ['play-large','progress','current-time','mute'], 
                                    fullscreen: { enabled: false, fallback: false } 
                                  });                                
                             }
                             if(res.data.current_video_caption)
                             {
                                $("#current_video_caption").val(res.data.current_video_caption);
                                $("#current_video_caption").fadeIn;
                             }
                               
                             
                             //end custom code short video 31th oct 2024
            ...
            submitPost: function(event) {
                ...
                view.mjobModel.set('remove_current_video',$("#remove-current-video").val());
                view.mjobModel.set('remove_current_video_id',$("#remove-video-id").val());
                view.mjobModel.set('short_video_attach_id',$("#short_video_attach_id").val());
