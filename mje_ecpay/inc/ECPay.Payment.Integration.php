...
class ECPay_CheckOutFeedback extends ECPay_Aio
{
    static function CheckOut($arParameters = array(),$HashKey = '' ,$HashIV = ''){
        ...
        if (!isset($arParameters['CheckMacValue']) ||  $CheckMacValue != $arParameters['CheckMacValue']) {
...
class ECPay_QueryTradeInfo extends ECPay_Aio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        ...
        // ecpay_log('ECPay_QueryTradeInfo Input Check: ');
        // ecpay_log('arParameters: ');
        // ecpay_log($arParameters);
        // 呼叫查詢。
        if (sizeof($arErrors) == 0) {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                // ecpay_log('arConfirmArgs: ');
                // ecpay_log($arConfirmArgs);
                // ecpay_log('hasKey: '.$HashKey);
                // ecpay_log('HasIV: '.$HashIV);
                // ecpay_log('EncryptType: '.$EncryptType);
                ...
                if ($szCheckMacValue != $szConfirmMacValue) {
                    //ecpay_log('check mack Fail.');
                    array_push($arErrors, 'CheckMacValue verify fail.');
                } else{
                    ecpay_log('check mack success.');
                }
            }
        }
...
if(! class_exists('ECPay_CreateTrade') ){
...
* 付款方式 : Google Pay
*/
if( !class_exists('ECPay_GooglePay') ){
